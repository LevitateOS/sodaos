...
Developers use native Forgejo account creation/onboarding; Soda has no account/password administration frontend. Soda's API/OAuth service uses the same configured Forgejo HTTPS origin under `/-/soda/`; the Sodaspaces drawer remains pending. Git authentication uses native Forgejo SSH keys or HTTPS tokens, independently of Soda development-access public keys.
