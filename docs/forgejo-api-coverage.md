...
| AD22 | Read complete native account-administration fields/MFA status, filter People by native status, explicitly reset another user's MFA | W `admin/users.go::{Users,ViewUser,EditUser,EditUserPost}` loads native MFA/detail/status-filter fields and `Reset2FA` deletes TOTP and WebAuthn credentials. A `admin/user.go::SearchUsers` only selects source/login/sort/page; D `User` omits MFA and several editable admin fields, and `EditUserOption` has no reset flag. Do not fetch all users and fabricate server-side filtering or use self-security authority. | 3 / D sensitive native admin contract |
