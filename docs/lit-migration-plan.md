...
bounded current layout, stable measured panes, shared chrome, compact native coexistence,
...
| Slots have stable local UUID keys; `selectSlot()` can invoke `restore()` | Keep owner keys stable when publishing an issued native ID. Do not use effectful selection for pure layout operations. |
| `soda-spaces:v1:<actor>` stores up to 64 locator entries and hidden flags, with a 16,384-code-unit read limit, but no pane tree or saved selection | Historical format; follow the current disposable-cache contract in step 5a. Browser cache changes do not authorize a DB reset. |
...
| Native tmux is per terminal ID | Follow the selected [terminal ownership contract](terminal-integration.md); no second PTY broker. |
...
| `…/sodaspaces-layout.ts` (step 5a) | Pure pane-tree transformations, current locator/layout parsing and serialization. Takes explicit measurements for geometry decisions; no DOM, storage IO, network, credentials or native lifetime effects. |
...
| Existing `internal/host/` and `project-os/` | Native account, allocation, tmux/systemd and exact cleanup authority under the [terminal contract](terminal-integration.md). |
...
current facade: `started`, `restore`, `disconnect`, `invalidate`, `dispose`, and
explicit Open/End controls. The terminal guide owns the selected native lifetime.
...
- Use the [current exact-ID HTTP End response](dashboard-api.md#exact-session-metadata-actions-and-creation-outcomes),
  never socket closure as cleanup proof. Preserve original-target confirmation.
...
The original registry/receipt slice's receipts remain in history. Current allocation,
native ownership/lookup/capacity and access requirements belong to the
[terminal guide](terminal-integration.md#managed-terminal-implementation-and-proof-limits),
not a second contract in this rendering plan. The selected removals replace request-ID
correlation, lifetime owners and cleanup receipts while retaining actual authorization,
writer exclusion, Stop admission and incomplete-collection handling.

Update production callers, bounded DTOs and test doubles together. Exercise multiple
independent IDs/projects, original actor/account checks, denied/degraded collections,
lost replies and native lookup, late Create versus End, expired/consumed permissions,
writer contention, cancellation and unavailable—not absent—outcomes. Source/double
checks are not native/CLI acceptance or a retained rollout.
...
- Extend `<soda-spaces>` from its current single environment into the shared workspace:
  ...
  explicit allocation/Create/rename, presentation-only Hide and confirmed End. Extract existing
...
#### 5a — layout model and current cache
...
- Use a typed binary split tree: stable pane IDs, axis/desired ratio and two children;
  ...
  most one leaf. The locator table holds exact native IDs scoped to their environment;
  live bindings/handles remain solely in the workspace.
- Preserve the local owner key while allocation publishes its one native ID.
  Publication updates one locator, not the host, tab or
...
- Follow the [current disposable-cache contract](terminal-integration.md#current-workspace-layout-and-continuity)
  for version/key, bounds, reset and storage failure. Validate unique IDs, canonical
  locators/references, finite ratios, node shape and selected membership. Empty panes
  consume UI structure, never native capacity. There is no old-format migration,
  pending namespace, malformed-byte custody or permanent save lockout. Losing an
  arrangement never creates or ends work; native inventory supports discovery.
...
**Exit:** round-trip tests cover exact IDs, hidden work, selection, ID publication,
duplicate/cross-project aliases, missing references, malformed trees/ratios, limits,
current-cache reset and storage failure. Emitted-browser reload/partial reads preserve
exact targets without Create; metadata refresh never replaces a live renderer.
...
- Put Rename, Hide and **End terminal…** in the selected
  ...
  Follow the exact native End outcome; closed sockets alone do not prove cleanup.
...
- Sessions/details/maximize/compact transitions and Hide/show are presentation-only.
  Follow the terminal guide for access/native lifetime; no layout callback sends a
  lifetime request. Unavailable results remain scoped and never trigger Create.
...
one unread flag per owner, observational lifecycle reasons, All/Attention
...
- Attention uses actual connection/cleanup observations, including unconfirmed
  outcomes—not retired deadline warnings. Keep connection loss, attached-elsewhere,
  ending, unconfirmed and acknowledged-ended distinct. Expose
...
- Add All/Attention and explicit Next attention to the same page/drawer navigation;
  ...
  as another user's session. Unavailable native observation is not absence.
...
and exact allocation/ID, writer, native-lifetime and cleanup checks. Add the actual
...
attachment expiry/helper loss continuity and uncertain outcomes under the current
terminal contract. Nothing authorizes unrelated cleanup.
...
| Storage/layout | Current-cache reset, issued-ID publication, hidden/unresolved records, stable selection/order, bounded data and storage failure. Pure transforms send no effects; reload/partial metadata verifies exact targets and no Create replay. |
...
2. **Candidate prepared and fixture delivery authorized:** use production exact-
   ...
   client state. Disposable browser layout is separate from persistent SQL migrations. Review preserved-state
