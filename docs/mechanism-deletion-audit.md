...
**Reading this after implementation:** the recommendations and estimate below
remain the original assessment. The [28-slice table](#all-28-slices--individual-disposition)
now reconciles them against source at `59e4da3`; it is not an all-complete receipt.
Open actions and explicit retention decisions belong to the
[implementation plan](refactoring-plan.md#removal-reconciliation-follow-up).
...
**Reconciled source: `59e4da3`.** Removed means the named policy/mechanism is absent;
replaced means responsibility remains with different machinery. Partial/open means
an identified gap, not completion. Retained means the original keep boundary still
has an actual caller/purpose. R1–R7 refer to the
[owning follow-up ledger](refactoring-plan.md#removal-reconciliation-follow-up).
These are source observations, not new native acceptance or a second independent
review. Prior checks retain their recorded scopes; no product tests were rerun for
this documentation-only reconciliation.

| # | Slice | Current source result | Original verdict and concrete boundary |
| --- | --- | --- | --- |
| 1 | Terminal lifetime policy | **Removed D1.** `create_terminal` starts systemd-owned tmux; `prepare` is bounded. Remaining heartbeats bound attachment IO, not shell lifetime. | **Delete D1.** Shell lifetime should not be renewed by browser authorization. Revoke IO without destroying work. |
| 2 | Terminal uncertainty accounting | **Replaced D2.** `terminal_sessions.go` has no lifetime/receipt maps; native reservation consumption and End share a lock. `terminal_status` observes exact ownership. | **Delete D2.** One preallocated ID and current native lookup replace the attempt namespace/cleanup-history monopoly. Keep in-flight exclusion and one writer. |
| 3 | Native terminal supervisor | **Guard removed.** `project_terminal.py` retains account/PTY, socket/cgroup and cleanup checks. Exact unit-attestation narrowing remains **R6**, not a completed deletion. | **Delete the D1 lease guard, not the whole Python program.** Account binding, privilege dropping, PTY behavior and exact native cleanup have separate purposes. |
| 4 | Terminal transport chain | **Retained deliberately.** `terminal.go`/host terminal bridge still carry bounded JSON/base64 across privilege boundaries. The separate detached owner stream is gone. | **Retain the privilege-crossing bridge.** The browser cannot directly access the root helper or container PTY. Removing JSON/base64 alone would be a protocol rewrite, not elimination of that boundary. D1 does remove the separate owner stream. |
| 5 | Browser terminal state machine | **D1/D2/D10 branches removed.** `sodaspaces-terminal.ts` reserves/publishes one ID, restores by inspection and uses original CSRF; renderer/IO/stale guards remain. | **Delete D1/D2/D10 branches.** Keep renderer lifetime, bounded IO, readiness, exact attachment, stale-callback exclusion and attach-only reconnect. A networked terminal still needs connection state. |
| 6 | Workspace layout machinery | **Retained deliberately.** `sodaspaces-workspace.ts`/`sodaspaces-layout.ts` still implement cross-project panes, tabs and stable live owners; no generic framework added. | **Retain.** Multiple simultaneous cross-project panes are requested functionality. One project's tmux cannot replace cross-container composition without introducing another connection layer. Flat DOM ownership prevents Lit moves from disposing xterm/socket owners; a generic docking framework would add machinery. |
| 7 | Workspace storage/compatibility | **D9 removed; D2 integrated.** `parseLayout` accepts v3 only; workspace resets invalid current cache and keeps later saves possible. Obsolete capacity wording remains in **R2**. | **Delete D9 and D2 dependants.** Keep one current, bounded layout and explicit native identity lookup. Layout preferences are not retained project data. |
| 8 | Attention/refresh | **Deadline attention removed.** `sodaspaces-attention.ts` projects output, unread and connection state, not retention deadlines. Observation refresh remains. | **Delete deadline-derived attention with D1; retain basic unread/connection indication.** `sodaspaces-attention.ts` is a small projection of real events, not an agent-status inference engine. No case for deleting all refresh/attention functionality was established. |
| 9 | Project creation/profiles/lifecycle | **D3/D6 coupling removed.** `apiCreateEnvironment` marks the persistent project ready before network application; project controls no longer keep a sticky uncertainty flag. | **Delete D6 and D3 coupling; retain real provisioning identity.** One persistent project per repository, original profile/CID, readiness and same-root lifecycle are not optional-network transactions. Broad profile deletion would remove selected product behavior. |
| 10 | Membership/SSH-key management | **Browser policy removed; native writer retained.** Project controls use original CSRF and consume each Apply preview. `project_keys.py` still owns account/revision/atomic-file safety. | **Retain native account/key integration; delete D3/D10 browser policy.** Forgejo profile keys are optional inputs, not authority to change project OpenSSH access. The managed writer's lock/revision/atomic publication protects actual key changes. Replacing everything with a Forgejo key import silently changes credential selection and revocation semantics. |
| 11 | Spaces collection/degraded reads | **Enumeration retained; D13 implemented.** `apiSpaces` keeps bounds/completeness and real authority. Repeated pre-operation `terminalCurrent` reads remain a conditional candidate, **R7**. | **Retain bounded authorized enumeration and partial-read semantics.** Membership/operator degraded reads serve persistent projects during provider trouble; they do not authorize a terminal. Repeated request-local reads can be simplified, but removing completeness/authority distinctions would hide failures or expose metadata. D13 addresses avoidable native read blocking. |
| 12 | Host Tailnet management adapter | **D8 veto removed.** No `verifyCLI`/`ManagementCLIRelease` chain remains. Native version in observation/revision and build pins are not live exact-release admission. | **Delete D8; retain host control adaptation.** CLI/LocalAPI commands cover real status/login/preference callers. Field-specific updates preserve native settings. The host revision fences stale drafts/identity, not provider ACL authority. No equivalent whole upstream browser-management replacement was demonstrated. |
| 13 | Enrollment policy/credential lifecycle | **Partial.** D4/D7 journals/archives are gone; `policy.json` atomically carries one credential. Always-false `EnrollmentVerified` and its consumers remain: **R1**. | **Delete D7 and D4's `ActiveRun`; retain one active policy and meaningful binding/revision checks.** Admission, new-project default and disconnect differ in effect. Collapsing them into one toggle would silently disconnect or retarget projects. The always-false `EnrollmentVerified` field is removable dead surface, not a subsystem. |
| 14 | Provider key-creation wrapper | **D4 journal removed; wrapper retained.** `keyTransport.used` limits one HTTP dispatch, not later explicit operations. Tag ordering/clock strictness remain **R5**; required shape checks stay. | **Retain a narrow bounded secret-handling adapter; remove D4-derived one-attempt policy.** The selected SDK performs one API call, but uses unbounded `io.ReadAll`; its OAuth convenience helper uses `context.Background()`. Wholesale substitution loses actual bounds. Single-use/ephemeral/tag/scope checks are meaningful; duplicate parsing, tag ordering and local-clock strictness are narrower simplification candidates, not justification to delete the broker. |
| 15 | Per-run enrollment journal | **D4/D6 runtime mechanisms removed.** `EnrollRun` writes no attempt record; `ProvisionProject`/network reservation callback is absent. Explicit retry follows node/exec observation. | **Delete D4 and D6.** Persistent attempt prohibition and the second Create reservation provide blocked recovery, not a required user capability. |
| 16 | Companion architecture/incarnation checks | **Partial follow-through.** `companionCreateArgs` uses `--state=mem:`; namespace/CID/image checks remain. Completed-resource retirement is missing (**R3**); recipe narrowing unresolved (**R6**). | **Retain the companion and real namespace/CID verification; delete only D5/D8 policy dependants.** Host-root Unix credentials do not automatically become privileged inside the shifted user namespace. Moving OAuth into a developer-controlled project or using a host-socket shortcut is not an equivalent deletion. Exact recipe comparison may be simplified only with its isolation assertions accounted for. |
| 17 | Runtime filesystem/DNS ownership | **Partial D5.** Node-state mount is gone; control/input and DNS backup remain meaningful. Old run directories/companion roots have no completed-run retirement path: **R3**. | **Delete D5 identity-preservation storage; retain restricted input/control ownership and DNS safety.** The companion and project share the actual resolver file. Ignoring inode/ownership conflicts can overwrite somebody else's DNS; memory node state does not eliminate resolver backup/restore. No Soda DNS server exists here to delete. |
| 18 | Start/Stop/restart orchestration | **Partial follow-through.** `parentStopQueued`/identity-preserving stop branch are gone. `stopTailnetRun` logs out/stops but does not retire old companion resources (**R3**). Native ordering remains. | **Delete D4/D5 recovery protocol; retain native parent-child lifecycle.** `BindsTo`/ordering, bounded startup/stop and exact-owned termination still matter. A cancelled Podman client is not proof its exec stopped consuming a key. That observation cannot simply be removed with the journal. |
| 19 | Tailnet UI/cross-feature projections | **Partial.** Pre/postflight choreography is gone, but `renderNetwork` still says never repeat uncertain key requests and requires a new project run: **R2** contradicts D4/D5. | **Delete D3/D8/D10 dependants; retain host/project separation, independent drafts and exposure confirmations.** Network status is not project readiness or proof of reachability. Removing those distinctions would misstate the product rather than remove an invented subsystem. |
| 20 | Runner command/dispatch layers | **Open conditional simplification, R4.** `Coordinator.mutate` and `Operations.mutate` still duplicate decode/ID validation/lifecycle dispatch. CLI administrator and socket admission remain distinct. | **Simplify, not a principal deletion.** `Coordinator` and `Operations` duplicate decoding/action switches, but the CLI and root socket remain real distinct callers with different admission/response needs. Consolidate a duplicated dispatcher only if it reduces code; do not rebuild an invoker hierarchy. The old runner subprocess hop is already gone and cannot be counted again. |
| 21 | Runner registration/stored state | **Retained deliberately.** `native_create.go`/`native.go` still install dedicated account, credentials/config and one-slot listener. No replacement scheduler/provider framework introduced. | **Retain.** Dedicated account, restricted token file, native Forgejo configuration and a systemd listener implement local capacity, which Forgejo's registration record does not install. The remaining one-value provider field is not a provider framework. No Soda scheduler, results store or multi-provider backend was found. |
| 22 | Runner inventory policy | **D11 removed.** `Native.List` retains readable rows and validated unavailable IDs; `Inventory.Response` qualifies counts. API/browser preserve nullable service and completeness. | **Delete D11.** A broken or unsupported saved runner must not suppress unrelated healthy local capacity. Per-target descriptor safety remains. |
| 23 | Runner lifecycle/uncertainty/confirmations | **D10/D12 removed.** UI and `apiRunnerAction` use strict `{}` for routine actions; Remove alone uses typed ID. Native descriptor/boot-policy checks remain. | **Delete D12 and D10 preflights; retain outcome/readback separation.** This UI already allows refresh after uncertainty; it is not the sticky project lockout in D3. Start/Stop's boot-policy coupling is explicit existing behavior, not a separate desired-state engine to delete. |
| 24 | Native-page connection/session coordination | **D3/D10 policy removed.** Native entry passes the validated session; standalone workspace bootstraps once. `soda-connection.ts` retains actual bootstrap/login/logout and actor retirement. | **Delete D10 and D3 propagation; retain bounded OAuth/session/logout coordination.** Inspected Forgejo 15.0.7 API middleware uses OAuth2/HTTP-signature/Basic (optionally reverse-proxy) authentication and calls `AuthShared` with no session store; forwarding the browser cookie is not a supported replacement. `login_contexts` makes logout win over a late consumed OAuth callback. Removing it without replacing that protection can resurrect a signed-out session. No borrowed Forgejo cookie or new authentication framework is proposed. |
| 25 | Privileged-helper admission/serialization | **D6/D13 removed.** `Daemon.ServeHTTP` lets the four fixed observations bypass writer admission; Create no longer calls the Tailnet reservation wrapper. Real writer locks remain. | **Delete D13 and D6 lock coupling; retain the root socket boundary and real writer locks.** Global read serialization is broader than needed; deleting all serialization would expose shared native mutation races. No new scheduling subsystem. |
| 26 | Repeated validation/state models | **D10 removed; ancillary work unresolved.** Boundary decoders remain deliberate. `projectKey` revalidates policy already validated by `load`; repeated collection admission reads remain (**R7**). | **Delete D10's duplicate browser authorization protocol; simplify redundant in-process semantic assertions individually.** Browser HTTP, privileged Unix socket, provider/native output and project filesystem are different boundaries. Deleting all decoders or replacing them with TypeScript casts is not a valid whole-mechanism removal. |
| 27 | Pre-release compatibility | **D9 removed; retained-state compatibility deliberately kept.** Current cache only; append-only database migrations and the singular-terminal 410 stub remain. No retained-state reset occurred. | **Delete disposable UI compatibility through D9; retain the existing database upgrade path.** Retained development fixtures are real state, including v9 databases; this candidate adds v10. `migrations.go` is an append-only migration list and bounded checks, not a general migration platform. Resetting/squashing it without a selected preserved-state transition is not supported. Tiny old-field/410 stubs are not a large deletion opportunity. |
| 28 | Tests/fixtures/packaging/procedural burden | **Partial.** Principal lifetime/retry/cache/runner/admission tests and installed source gates were ported, but R1 fixtures and R2 copy escaped; R3 has no retirement implementation/evidence. | **Delete support for each retired mechanism with its source.** Port useful behavior tests; retire assertions about the deleted deadlines, phases, archive copies, legacy formats and universal lockouts. Keep real authorization, state preservation, native topology and browser asset coverage. No new evidence framework, full-suite ritual or unrelated architecture gate is proposed. |
