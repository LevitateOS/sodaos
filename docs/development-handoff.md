# Retained development handoff

This handoff preserves the unrelated development workstreams, retained-target state
and scoped permissions moved out of `implementation-status.md` at the owner's request.
The [implementation status](implementation-status.md) now exclusively tracks the
single-run build/release replacement. Moving these records does not select, complete
or authorize further work on these targets.
...
- **Release engineering:** current grants and protected signing custody are owned
  by the [release implementation status](implementation-status.md#current-permissions),
  not this development-target handoff.
...
- Apart from the separately owned release-signing/candidate-publication grant, there is
