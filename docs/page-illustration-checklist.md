...
- User-requested robot consistency edit replaces both characters in `home-papercraft.png` with the dashboard identity, removing robot antennas while retaining the server antenna and shared-workstation composition. Exact built-in generation prompts: `assets/branding/forgejo/home-art-prompt.md`. Verified 1536×1024 RGBA, alpha 0–254; native desktop capture `.artifacts/screenshots/capture-gq51pj/001.png` inspected with clean transparency and no overlap. Earlier captures show the previous character variant. This asset is also used by setup; setup remains unverified on an uninstalled instance. No templates/CSS changed; no automated tests rerun for this image-only edit.
