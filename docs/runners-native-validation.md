...
| `tests/installed/runners-native.ts` and `runner-state.py` | Pinned SSH transports the actual product-owned read-only observer on stdin. It verifies native hostname/architecture, CLI inventory, accounts, credential modes/hashes, effective confinement and selected state hashes. Partial inventory remains useful in the UI but is not a complete retained-state baseline: the observer/postconditions require `complete:true`. No arbitrary remote program, account/path selector, native mutation or repair operation is accepted. |
