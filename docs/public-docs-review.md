...
| --- | --- |
| ISO/QCOW2/Scaleway | Website launch paths are preserved. The [CoreOS ISO](coreos-installer.md) has bounded generation/diskless boot evidence; complete fresh installation and release delivery remain unvalidated. The [media direction](installation.md#publication-direction) selects ISO as primary, recommends a prepared QCOW2 next and includes the matching Soda payload; the [manual-install correction](coreos-installer-plan.md) removes the key prompt in favor of a root password. Those changes remain unimplemented; cloud delivery remains unselected. Bind the public guides to the completed first-boot flow, exact artifacts, firmware/disk semantics and platform-specific provisioning before launch. Do not call the test CoreOS base a preinstalled Soda disk or infer cloud-init/Anaconda from the predecessor. |
