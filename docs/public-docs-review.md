...
| --- | --- |
| ISO/QCOW2/Scaleway | Website launch paths are preserved. The [CoreOS ISO implementation](coreos-installer.md) has source/local tests; media generation/boot and release delivery remain unvalidated. QCOW2/cloud deliverables remain unselected. Bind the public deployment guides to real release recipes, exact artifact forms, target firmware/disk semantics and platform-specific Ignition delivery before launch. Do not call a CoreOS base/kit a preinstalled Soda disk or infer cloud-init/Anaconda from the predecessor. |
