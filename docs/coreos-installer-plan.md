...
- **Locally available apps after download:** embed all five exact images in the
  ...
  defines the single shared OCI layout; obsolete payload readers are removed.
...
define the selected native client-trust contract. This packaging route does not
implement the former custom signed-channel client admission contract.
...
host manifest. The wizard verifies all five images in the shared OCI layout and retains the existing
