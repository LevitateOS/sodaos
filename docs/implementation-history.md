...
“remains v6” were recorded at that time; they do not renew permissions, reopen
completed work or override the current status. Successful results do not make the
recorded sequence a recommended workflow. Failures and original receipts remain
unchanged below. Paths into subsequently removed worktrees are historical references,
not claims that those outputs are still retained.
...
a new isolated stock Forgejo localhost:3300 fixture, its non-admin account and
restricted fixture inputs, and existing initialization/auth/browser/OAuth tests.
Pulled `0f1d2b1`/`9e21efc` by fast-forward from a clean tree. Read-only inspection
found port 3300 free and no `sodaos-local-forgejo`; unrelated running services and
all retained stopped fixtures were left untouched. Installed official Google Chrome
and prepared the pinned workspace's Playwright Chromium. No global browser CA trust,
host network policy, appliance deployment or real runner/provider job was changed.

Created the loopback-only rootless `sodaos-local-forgejo` from stock Forgejo 15.0.7
and new named volume `soda-pages-0f1d2b1-data`. Supported read-only template/public
mounts use frozen `0f1d2b1`; their templates compare identical to the final candidate.
Public assets were generated by `bun run build:preview`, not invented native HTML.
Configuration uses SQLite, installation lock, disabled public registration/SSH,
localhost root URL and Soda light/dark themes (default light). Generated the
non-admin `soda-screenshot` password with Forgejo's own `--random-password` CLI,
redirecting output directly to restricted ignored input; no secret in argv/output.
Created its fixture-only public initialized repository `soda-screenshot/page-fixture`
(ID 1) through the supported API with credentials read internally, not logged.
The fixture, volume, account, repository, four OAuth applications and every attempt
are retained. This is not any existing retained appliance/provider fixture.

### Reproduced fixture failures and fix

First `0f1d2b1` page run failed because repository ID 1 was absent. After the bounded
fixture initialization, all 16 consumers passed, but the real parent encountered
an unexpected helper POST `/profile` and timed out waiting for native repository
settings. Unlike the Mac's existing repository, this fresh repository belongs to
`soda-screenshot`: its owner view legitimately reads creation metadata. Extended
only the existing Go fixture's fixed read-only helper double to return the existing
`testCreationProfile()` response for `/profile`; every other unexpected operation
still fails. No second harness, native HTML mock, production behavior change or
real project creation followed. This profile/list data and consumer operation
interceptions remain synthetic, not installed helper/process proof.

Fix and exact frozen final candidate:
**`2cdc23861de5829bb0010e292475b9e7c35add6f`**.
Worktree: `.artifacts/worktrees/combined-candidate-2cdc238/`.
Fixture input: that worktree's `.local/screenshot-fixture/create-output.txt`, mode
0600 under restricted directories, explicitly copied from the same newly created
fixture's `0f1d2b1` worktree input. Do not replace it with Mac/other-fixture credentials.
There is no saved manual screenshot profile on this builder; the test harness owns
its separate retained browser state.

### Fresh final-candidate commands and actual evidence

With isolated Go 1.26.7 (`.artifacts/runners-step4-continued/toolchain/go/bin` on PATH),
`GOTOOLCHAIN=local` and Bun 1.4.2:

- `bun run test:pages`: passed all 16 consumers and the real native parent,
  including populated runners and native profile-draft Back/BFCache/Forward.
- `bash scripts/build-native.sh x86_64`: fresh native build/seal passed, 30.29s.
- `bash scripts/check-native.sh x86_64`: **full gate passed**, 209.25s. Go/module,
  strict TypeScript/Lit, 218 frontend tests (16 initial gated skips), repeated
  mandatory page gate (all 16 consumers plus real parent), one layout test,
  36 Forgejo tests (22 gated/optional skips), 60 Cockpit tests, 141 Python build
  tests (two optional skips), 11 staging tests and final sealed integrity passed.
  The catalog-mismatch stderr is an expected negative Python fixture, not a failed
  aggregate. Optional skips remain explicit; no required consumer was skipped.
- Existing `tools/soda-artifacts bundle --source .../.artifacts/native/x86_64
  --arch x86_64 --revision 2cdc23861de5829bb0010e292475b9e7c35add6f --out .../export/x86_64`
  passed, followed by an independent `verify` of that exported tree.

Verified export: **`.artifacts/combined-candidate-2cdc238/export/x86_64/`**.
SHA-256 of its `SHA256SUMS`:
`1193caeca27d5c4d098fe339254c6862bdbab24a74876e2c372c874c9218c47f`.
This identifies the existing manifest chain, not a signature/reproducibility claim.

Evidence `.artifacts/combined-candidate-2cdc238/`: `pages.log`, `build.log`,
`check.log`, `export.log`, `export-verify.log`, `export-manifest-sha256.txt`,
`paired-sha256.txt`, `paired-build-info.txt`, `tools-revision.txt`,
`mounted-template-comparison.log`, and browser version logs. Actual versions:
Linux x86_64, Go 1.26.7, Bun 1.4.2, Playwright 1.63.0, Playwright Chromium
153.0.8010.12 and Chrome 153.0.8010.36. The native parent uses sandboxed Chrome and
fixture-only TLS SPKI; sandboxed launches independently confirmed browser versions.
Successful private page evidence is retained in the final worktree at
`.artifacts/pages-gPTucN/native/` (focused) and `.artifacts/pages-dUtoJV/native/`
(full gate). Earlier install/preparation/failure logs remain in
`.artifacts/combined-candidate-0f1d2b1/`; failed private fixtures remain in its
worktree at `.artifacts/pages-iHaBmI/` and `.artifacts/pages-HwolxZ/`.

**Step 3's bounded x86_64 build/check/export is complete.** This is not installed
runner/provider execution, whole-product/native/aarch64 acceptance or permission
to deploy. Step 1's wider genuine predecessor-cache, visual and authority gaps and
step 2's exact target/provider proposal remain separate. Both-media geometry checks
are not a screenshot/complete theme review. No real runner registration, job,
service lifecycle, retained-project mutation, reboot, cleanup or delivery occurred.
...
existing-root OS observation and native-tooling work. The only textual conflict was
independently prepended handoff entries; both are preserved below. Incoming changes
are documentation only. This merge checked whitespace, conflict-marker removal and
preservation of both parent handoffs; it did not rerun product tests or perform a
build/deployment. Earlier checks remain evidence for their recorded source bytes.
