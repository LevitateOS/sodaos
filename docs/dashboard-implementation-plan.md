...
| --- | --- | --- |
| U01 | Full H01 source action register; development baseline/support research and draft auth/read contracts | Reviewed supported baseline, field-complete first contracts, authentication/lifecycle decisions and maintenance/license ownership; continue per-feature conformance, not another broad inventory |
...
1. **Close U01 using its delta-only checklist:** reuse H01, the source guide and existing auth/read drafts. Resolve outstanding decisions and explicitly identified evidence gaps; do not repeat the v16 API comparison, source preparation or broad audit. Neither inspected version is already approved for an upgrade.
...
1. **Close, do not restart:** follow U01's remaining-delta checklist, resolving the existing baseline/maintenance/auth/read/lifecycle drafts in parallel. U07 terminal design remains separate before its implementation. No second inventory, authentication proposal or source preparer.
...
- [Authentication draft](forgejo-authentication-design.md): native-owned transaction direction, sequence, threat model, WebAuthn constraints and explicit questions. Edit this draft to resolve those questions, not author another authentication proposal.
...
#### Remaining closure work — only the delta

| Step | Existing input | Remaining action / output | Owner |
| --- | --- | --- | --- |
| 1. Baseline disposition | Source guide's v16 recommendation and recorded support/backport tradeoff | Release-note retrieval/review and engineering disposition are now recorded below and in the source guide. Finish only human support/adoption disposition; refresh decision-critical support facts if stale or the version changes. Do not repeat retrieval or the API comparison. | U01 with U02/U03 and human maintainer |
| 2. Build/license readiness and maintenance | Existing source guide, real manifests/locks and upstream recipe inspection | Identify only unreviewed dependency/notice obligations and unresolved build/runtime inputs; record their dispositions, source/notices delivery contract and named maintenance/security reviewer. Put actual image pin resolution, recipe wiring and distribution assembly in U02; no new dependency inventory detached from the real locks. | U01/U02; human assignment required |
| 3. Authentication decisions | Existing H05 draft and its four decision questions | Origin/RP-ID configuration policy and the first wire/state/binding/lifetime/abuse candidate are now recorded. Resolve external IdP/logout policy and named security ownership; review that candidate against exact native helpers, investigating only an unanswered feasibility question. Do not write a replacement draft. | U04/U05/U16, reviewed in U01 |
| 4. First read/compatibility contracts | Existing read draft and source findings | DTOs, predicates, errors, byte encoding, limits, freshness and Linux primitive evidence are now recorded. Review the concrete candidate; retain explicit native/parser/packaged-runtime test requirements. Implementing/fixing/proving the full slice belongs to U02/U03/U09, not another U01 definition pass. | U03/U04/U09, reviewed in U01 |
| 5. Association consequences | H05 linked-identity section, existing stable identities/project-local logins and audited native lifecycle actions | Decide the remaining rename/transfer/disable/delete/revocation effects on Soda records and new browser access. Preserve existing roots/accounts/data and separate native authorities; no automatic Linux remapping/offboarding. Record decisions in the existing design/owning feature sections, not a new lifecycle subsystem. | U05/U07/U12/U16 |
| 6. Close U01 | Decisions recorded above and existing 179-group ownership | Reconcile concrete review dispositions and named human responsibilities, link later feature-specific work/tests to its existing U owner, and record U01 acceptance or the exact unresolved blocker in the handoff. Do not require another broad audit or full native product acceptance. | U01 with affected owners |
...
- **First contracts now concrete candidates:** existing auth/read drafts now specify initial routes, fields, state/binding, errors, resource/attempt limits and compatibility freshness. A bounded local Git/worker experiment supports the Linux limit-launcher primitive; it does not prove Forgejo, packaged Alpine, worst-case behavior or aarch64. Native security review, implementation and contract tests are still required, not another proposal or inventory.
...
- **Association policy narrowed:** stable-ID rename/name-reuse, unavailable lookups, confirmed browser invalidation and preserved Linux data/access consequences are recorded. Project administration currently uses stored `Project.OwnerID`; transfer succession—including an organization recipient—is a real unresolved product decision, not already implemented dynamic ownership.

**Consolidated human decisions (answer these, do not research them again):**

1. Who accepts stable-release/security maintenance and native-auth review responsibility? Do not assign the user or an upstream team without confirmation.
2. Which license should cover original SodaOS code, subject to retained third-party rights? No top-level grant exists; do not silently add one or relicense Forgejo.
3. May a configured external IdP display its own login/consent, returning to Soda, while every Forgejo-owned page stays in Soda? Recommended: yes. Also confirm ordinary sign-out clears this Soda session, with separately labeled native grant/session revocation rather than silently revoking all application sessions.
4. After a repository transfer, who administers its associated project: the original designated human, the new human owner, or an explicit handover recipient? Specify the organization-recipient case too. Linux homes/accounts/data are preserved in every case; site/org administration never implies host authority.

**U01 disposition:** not accepted. Human assignment/policy/license decisions and the first native-auth/license review dispositions remain open. The original release-note/metadata gaps and initial wire-definition work above are no longer blank tasks. Do not label this as full licensing clearance, native conformance or completed U02–U09 implementation.

**How to run the remaining checklist:** present the human decisions together as above. Technical gap-filling can proceed in parallel; do not spend another research pass on facts awaiting a decision. Before any new investigation, name the unresolved question, the existing evidence it cannot answer and the smallest expected result. Reopen settled research only for changed source/configuration, stale decision-critical information or contradictory evidence, and record that reason. Preserve earlier evidence and failures.
...
**Next work:** execute U01's delta-only closure checklist above using the existing research and drafts; resolve decisions and only explicitly missing evidence. Then extend H02's existing source preparer into the native build spine and implement U03/H03 with the first U09/H04 slice and reviewed U04 authentication path. Continue independent stock-adapter/UI corrections. Keep U08 accepted, every other unfinished U owner accountable, and U17/U18/U20 gated on complete delivered behavior—not another audit, a version bump or successful bundling alone.
