...
| --- | --- | --- | --- |
| 1. Baseline disposition | Source guide's v16 recommendation and recorded support/backport tradeoff | Release-note retrieval/review and engineering disposition are now recorded below and in the source guide. Apply the recorded upstream-versus-Soda maintenance boundary; resolve only actual patch/build adoption questions and refresh decision-critical support facts if stale or the version changes. Do not repeat retrieval or the API comparison. | U01 with U02/U03 and human maintainer |
| 2. Build/license readiness and maintenance | Existing source guide, real manifests/locks and upstream recipe inspection | Identify only unreviewed dependency/notice obligations and unresolved build/runtime inputs; record their dispositions, source/notices delivery contract and named maintenance/security reviewer. Put actual image pin resolution, recipe wiring and distribution assembly in U02; no new dependency inventory detached from the real locks. | U01/U02; review of Soda's own integration and delivery |
| 3. Authentication delegation | Corrected H05 delegation contract and existing native findings | Preserve native authentication/IdP/consent/logout/revocation semantics. Review only the missing interface, transport binding and native shared-service work; do not ask the user to choose upstream policy or implement the withdrawn custom account/factor rules. | U04/U05/U16, reviewed in U01 |
...
| 5. Association consequences | H05 linked-identity section, existing stable identities/project-local logins and audited native lifecycle actions | Implement conforming stable-ID/native-authority lookup and propagate confirmed native outcomes to legitimate Soda records/access. Remove stale-owner reliance; preserve roots/accounts/data and distinct authorities. Review an actual interface/mapping gap, not an alternative transfer policy or Linux offboarding subsystem. | U05/U07/U12/U16 |
...
**User clarification now governs:** Forgejo owns its policies and upstream fixes; Soda extends/delegates and maintains its own code/patches. Preserve native external-IdP/logout/transfer behavior rather than asking the user to redesign it. The user selected **Apache-2.0 for original SodaOS code**; retain all third-party licenses, including Forgejo's terms. Authoring the original-code license/notice boundary and completing dependency compliance remain delivery work, not an unanswered license-selection question.

**U01 disposition:** not accepted. Concrete native-interface/security review and license/build readiness remain; the former questions about owning upstream policy, alternative logout/transfer rules and license selection are removed as blockers. Completed research is not reopened, and no native conformance or completed U02–U09 implementation is claimed.

**How to run the remaining checklist:** implement/review the conforming interfaces and the specific corrections below. Technical gap-filling can proceed in parallel; do not spend another research pass on facts awaiting a decision. Before any new investigation, name the unresolved question, the existing evidence it cannot answer and the smallest expected result. Reopen settled research only for changed source/configuration, stale decision-critical information or contradictory evidence, and record that reason. Preserve earlier evidence and failures.
