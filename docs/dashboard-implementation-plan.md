...
| --- | --- | --- | --- |
| 1. Baseline disposition | Source guide's v16 recommendation and recorded support/backport tradeoff | Obtain the complete missing release-note material (the retained response is truncated), inspect only uncovered migration/security implications, and record adopt/reject with reasons and upgrade constraints. Refresh time-sensitive support facts only when needed for the actual decision. Do not repeat the API comparison. | U01 with U02/U03 and human maintainer |
...
| 3. Authentication decisions | Existing H05 draft and its four decision questions | Resolve production origin/RP-ID preservation, external IdP browser interaction, logout/revocation semantics and human security ownership. Complete the first protocol's wire/state/binding/lifetime/abuse details in that draft. Investigate only a concrete unanswered feasibility question; bring an actual architecture conflict back for a decision. | U04/U05/U16, reviewed in U01 |
| 4. First read/compatibility contracts | Existing read draft and source findings | Finalize exact DTOs, native predicates, errors, byte encoding, limit behavior, compatibility freshness/failure rules and justified work/time/output/concurrency budgets. Use a bounded local experiment only where a particular budget or feasibility claim lacks evidence. Specify the parser/cancellation fixes and acceptance tests; implementing/fixing/proving the full slice belongs to U02/U03/U09. | U03/U04/U09, reviewed in U01 |
