...
**Research date:** 12 September 2026. **Status:** reference research, not active work. The user [chose to retain Forgejo](../architecture/trust.md) rather than introduce OneDev's Java/JVM server environment. No OneDev plugin proof or replacement is selected.

**Scope if reconsidered:** The user clarified **zero users and zero deployed installations**. The [pre-release replacement scope](../product/scope.md) excludes a customer migration programme. Development fixtures do not create a legacy support obligation.
...
Current product requirements remain with [Architecture](../architecture/overview.md), [Project OS](../reference/project-os.md), [native pages](../reference/forgejo.md), [Runners](../reference/runners.md), and [Services/AI](../product/scope.md). This report evaluates those requirements; it does not silently change them. Installed state and permissions remain in the [handoff](../development/README.md).
...
The factual support description in the [customization guide](../reference/forgejo.md#shared-presentation-components) is corrected with this research. Existing selected routes and security contracts are not changed by that correction.
...
These options apply only if the [forge decision](../architecture/trust.md) is reopened. They are not a current task list or execution approval. The [handoff](../development/README.md) continues to govern fixture, host and provider effects.
