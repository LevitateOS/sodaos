...
Caddy sends only `/-/soda/avatars/*` on the configured Forgejo origin to the
existing backend at `127.0.0.1:8080`, preserving the path and dropping Cookie and
Authorization for that route. Other Forgejo routes still reach port 3000.
Existing Soda API/OAuth endpoints retain their distinct configured origin.
This narrow image route does not implement the Sodaspaces session/origin migration.
