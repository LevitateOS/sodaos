...
The original handoff was **source-complete, unbuilt, unvalidated (M01–M14)**. The user subsequently authorized local builds/tests and scoped native execution on the existing isolated `soda-test` VM, using this x86_64 infra workspace as builder/client. Candidate `c96c108` passed full native build/check and backed-up populated-v3 rollout at `/app/`; default routes remain HTMX. Four environments are retained. The explicitly approved project-namespace SYS_PTRACE correction and further fresh fixture now pass different-UID workload exec with member denial. Preceding project/VM lifecycle comparisons preserved the earlier three roots; the follow-up did not repeat lifecycle operations. U08 remains unaccepted pending host/operator regression and exact-revision lifecycle reconciliation. Both additional-fixture approvals and the original VM reboot have been used; do not infer another fixture, capability change, reboot or target. Preserve every root, later write, private credential input and evidence. The private route serves infra, not the laptop automatically; old backups are not lossless rollback. See `docs/local-testing.md` and `docs/implementation-status.md` for exact bytes, evidence and remaining scope.
