...
export function RepositoryDetail({ session }: { session: Session }) {
  ...
  return <section aria-busy={busy}><h1>{owner}/{repo}</h1>{error && <Alert isInline variant="danger" title={error} />}
    ...
    {repository && <><p>{repository.description}</p>
      <nav aria-label="Repository navigation"><Link to={`${repositoryPath(owner, repo)}/releases`}>Releases</Link><Link to={`${repositoryPath(owner, repo)}/actions`}>Actions</Link><Link to={`${repositoryPath(owner, repo)}/settings`}>Settings</Link><Link to={`${repositoryPath(owner, repo)}/pulls`}>Pull requests</Link><Link to={`${repositoryPath(owner, repo)}/issues`}>Issues</Link><Link to={`${repositoryPath(owner, repo)}/history?${new URLSearchParams({ ref: ref || repository.default_branch, path })}`}>History</Link><Link to={`${repositoryPath(owner, repo)}/refs`}>Branches and tags</Link><Link to={`${repositoryPath(owner, repo)}/compare`}>Compare</Link><Link to={`${repositoryPath(owner, repo)}/fork`}>Fork</Link><Link to={`${repositoryPath(owner, repo)}/edit?${new URLSearchParams({ ref: ref || repository.default_branch, new: "1" })}`}>Create or upload file</Link></nav>
