...
export function Hooks({ session }: { session: Session }) {
  ...
  return <section aria-busy={busy}><h1>Repository webhooks: {owner}/{repo}</h1><Link to={repositoryPath(owner, repo)}>Repository</Link>{error && <Alert isInline variant="danger" title={error} />}{!items && !error && <Spinner aria-label="Loading hook metadata" />}
    ...
    </Form><p>Metadata edits follow native last-write behavior. The pinned REST PATCH cannot rotate signing secrets or change package/action event flags; use <a href={`${session.forgejo_url}/${encodeURIComponent(owner)}/${encodeURIComponent(repo)}/settings/hooks`}>native Forgejo hook settings</a> for those operations and other provider-specific hook creation. No hook test or delivery replay is triggered here.</p>
  </section>;
}
