...
interface Detail { environment: Environment; observed: Observed | null; native_unavailable: boolean; authority_unavailable?: boolean; login: string; environment_administrator: boolean }
...
export function EnvironmentDetail({ session }: { session: Session }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        const memberResult = await request<{ items: { user_id: string; login: string }[]; authority_unavailable?: boolean }>(`${api}/members`, { signal: controller.signal });
        if (controller.signal.aborted) return; setMembers(memberResult.items);
        if (memberResult.authority_unavailable) setDetail({ ...result, environment_administrator: false, authority_unavailable: true });
        if (result.login) {
          try {
  ...
  return <section aria-busy={busy}><h1>{detail?.environment.name ?? "Environment"}</h1><code>{id}</code>
    ...
    {detail && <><p>Associated repository: {detail.environment.repository}</p>
      ...
      {detail.authority_unavailable && <Alert isInline variant="warning" title="Current Forgejo ownership could not be verified. Only your own membership is available; existing Linux access has not been changed." />}
