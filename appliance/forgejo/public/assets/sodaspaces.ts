...
export interface DrawerContent { refresh(): void | Promise<void>; dispose(): void }
export function mountDrawer(doc: Document, mountContent: (root: HTMLElement, context: DrawerContext) => DrawerContent = mountSodaspaces) {
  ...
  if (subUrl !== '' || !identifier(repositoryId) || (signed !== 'true' && signed !== 'false') ||
  ...
  const close = root.querySelector<HTMLElement>('#sodaspaces-close'), content = root.querySelector<HTMLElement>('#sodaspaces-content');
  const divider = root.querySelector<HTMLElement>('#sodaspaces-divider');
  ...
  root.dataset.mounted = 'true'; rows.item(0).append(button); root.hidden = false;
  let controller: DrawerContent | undefined; let ended = false, width = 50;
  const size = () => {
