// Soda owns authorization/lifetime; tmux owns live state. A stored ID is only a
// locator. Never store credentials, replay input or turn failed attach into create.
const identifier = value => typeof value === 'string' && /^[1-9][0-9]*$/.test(value) && BigInt(value) <= 9223372036854775807n;
const terminalID = value => typeof value === 'string' && /^[0-9a-f]{32}$/.test(value);
...
export function mountTerminal(root, context, loadRenderer = renderer) {
  ...
  const doc = root.ownerDocument, win = doc.defaultView, lifetime = new win.AbortController();
  const storageKey = `soda-terminal:${expectedUserId}:${environmentId}`;
  let id, uncertainCreate = false;
  ...
  const remember = value => { try { if (value) win.sessionStorage.setItem(storageKey, value); else win.sessionStorage.removeItem(storageKey); } catch { /* locator only */ } };
  let state = 'idle', disposed = false, generation = 0, socket, terminal, fit, observer, timer, request, retries = 0;
  ...
  const button = (text, primary = false) => { const b = doc.createElement('button'); b.type = 'button'; b.className = `ui ${primary ? 'primary' : 'basic'} button`; b.textContent = text; return b; };
  ...
  const detach = (message, stale = false) => {
  ...
  async function json(path, session, body, signal) {
    const headers = {'X-Soda-Expected-User-ID': expectedUserId};
    if (body) { headers['Content-Type'] = 'application/json'; headers['X-CSRF-Token'] = session.csrf_token; }
  ...
  async function session(signal) {
    ...
    if (value.user?.id !== expectedUserId || typeof value.csrf_token !== 'string' || !value.csrf_token || value.forgejo_url !== win.location.origin) throw Error('session');
    return value;
  }
  async function retention(action, seconds) {
    ...
    try {
      ...
      if (action === 'end') {
        ...
        if (result.terminal?.id !== target) throw Error('outcome');
        status.textContent = result.terminal.retain_until ? `Retained until ${new Date(result.terminal.retain_until * 1000).toLocaleTimeString()}, or authentication expiry.` : `Active as ${login}; authentication and native safety leases still apply.`;
      }
  ...
  const resize = () => {
    if (!terminal || state !== 'ready' || !screen.isConnected || !screen.clientWidth) return;
    fit.fit(); if (terminal.cols >= 2 && terminal.cols <= 500 && terminal.rows >= 2 && terminal.rows <= 300) send({type: 'resize', cols: terminal.cols, rows: terminal.rows});
  };
  async function connect(automatic = false) {
    ...
    try {
      ...
      if (own.environment?.id !== environmentId || own.environment.repository_id !== repositoryId || !own.environment.provisioned || own.login !== login) throw Error('membership');
      const metadata = await json(`/api/environments/${environmentId}/terminal-session`, null, null, request.signal); if (!live(n)) return;
      const existing = metadata.terminal;
      ...
      if (!id && existing) { id = existing.id; uncertainCreate = false; remember(id); }
