...
def fetch(out):
    ...
    for item in json.loads((ROOT / 'appliance/terminal-assets.lock.json').read_text()):
        if all(
            (out / f['file']).is_file() and hashlib.sha256((out / f['file']).read_bytes()).hexdigest() == f['sha256']
            for f in item['files']
        ):
            continue
        ...
        if (
            len(body) > 10_000_000
            or 'sha512-' + base64.b64encode(hashlib.sha512(body).digest()).decode() != item['integrity']
        ):
