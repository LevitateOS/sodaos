...
func TestForgejoOrgDetailsSourceParityAndParse(t *testing.T) {
	cases := []struct {
		...
		{"{{template \"org/header\" .}}\n\t<div class=\"ui container soda-page-container soda-org-details-intro\">{{$artwork := \"\"}}{{if .PageIsOrgTeamsNew}}{{$artwork = \"new-team-papercraft.png\"}}{{end}}{{template \"custom/soda/page_intro\" dict \"TitleID\" \"soda-org-detail-title\" \"Title\" .Title \"Class\" \"soda-page-intro--compact\" \"Artwork\" $artwork}}</div>", "{{template \"org/header\" .}}"},
