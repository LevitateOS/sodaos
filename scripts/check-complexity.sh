...
# internal/, cmd/, appliance/, and project-os/ must stay below 10 (gocyclo
# flags 10 and above). tools/, scripts/, tests/, and *_test.go are out of
...
shipping() {
  printf '%s\n' "$@" | grep -E '^(internal|cmd|appliance|project-os)/' | grep -v '_test\.go$' || true
}

if [ "$#" -gt 0 ]; then
  files=$(shipping "$@")
else
  files=$(find internal cmd appliance project-os -name '*.go' ! -name '*_test.go')
fi
if [ -z "${files:-}" ]; then
