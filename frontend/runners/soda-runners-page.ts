...
const unconfirmed =
  'Operation unconfirmed: local account, files or listener may have changed. Inspect local and Forgejo state before retrying. Refresh does not confirm the operation; no automatic retry or rollback occurred.';
const notSent = 'Operation was not sent. Check authorization and refresh before trying again.';

function noticeForRunnerError(error: unknown, sent: boolean): string {
...
function confirmationButton(event: Event): HTMLButtonElement | null {
  return event.currentTarget instanceof HTMLButtonElement ? event.currentTarget : null;
}
...
class SodaRunners extends LitElement {
  ...
  private retire() {
    ...
    if (this.operation?.kind === 'runner') {
      this.notice = this.operation.sent ? unconfirmed : notSent;
    }
    this.operation = null;
    this.pending = null;
  ...
  private async refresh() {
    ...
    try {
      await this.load(lifetime);
    } catch {
      if (this.current(lifetime))
        this.message =
          'Runner inventory unavailable. Previous observations are stale, not an empty or healthy inventory.';
    } finally {
      if (this.current(lifetime)) this.busy = false;
    }
  }
  private canMutate(lifetime: AbortController | null): lifetime is AbortController {
    return !this.busy && this.current(lifetime) && !this.stale && !this.blocked;
  }
  private async finishMutation(lifetime: AbortController) {
    ...
    if (!this.blocked) {
      try {
        ...
        if (this.current(lifetime)) this.message = 'Local refresh failed; shown observations are stale.';
      }
    }
    if (this.current(lifetime)) this.busy = false;
  }
  private async mutate(path: string, body: string) {
    const lifetime = this.lifetime;
    if (!this.canMutate(lifetime)) return;
    this.busy = true;
    this.pending = null;
    this.confirmationTrigger = null;
    ...
    try {
      ...
      if (this.current(lifetime)) this.notice = noticeForRunnerError(error, this.operation?.sent === true);
    } finally {
      body = '';
  ...
  private pendingMatches(runner: string, action: LifecycleAction): boolean {
    return this.pending?.id === runner && this.pending.action === action;
  }
  private focusPendingConfirmation(lifetime: AbortController, runner: string, action: LifecycleAction) {
    if (!this.current(lifetime) || !this.pendingMatches(runner, action)) return;
    this.querySelector<HTMLElement>(confirmationFocusSelector(action))?.focus();
  }
  private async chooseAction(event: Event, runner: string, action: LifecycleAction) {
    const lifetime = this.lifetime;
    if (!this.canDispatchAction(lifetime)) return;
    this.confirmationTrigger = confirmationButton(event);
    this.pending = {id: runner, action};
    await this.updateComplete;
    this.focusPendingConfirmation(lifetime, runner, action);
  }
  ...
  private runnerView(row: Runner, disabled: boolean) {
    return html` <article class="settings-runner">
      ...
      <p>
        ${row.service ? html`Service: ${row.service.load} / ${row.service.active} / ${row.service.sub}. Boot policy: ${row.service.enabled}.` : 'Service and boot policy unavailable.'}
        ${row.capacity} configured slot.
      </p>
      ...
      <div class="settings-actions">
        ${(['start', 'stop', 'restart', 'remove'] as const).map(
          (action) => html` <button
            ...
            ?disabled=${disabled}
            @click=${(event: Event) => void this.chooseAction(event, row.id, action)}
          >
            ${action} ${row.id}
          </button>`
        )}
  ...
  private inventoryView(disabled: boolean) {
    ...
    return html` <section aria-label="Local runner inventory">
      ...
      <dl class="settings-capacity">
        <div>
          <dt>Observed runners</dt>
          <dd>${this.rows.runner_count}</dd>
        </div>
        <div>
          <dt>Known listening services</dt>
          <dd>${this.rows.active_listeners}</dd>
        </div>
        <div>
          <dt>Known configured slots</dt>
          <dd>${this.rows.total_capacity}</dd>
        </div>
      </dl>
      <p class="settings-help">
        Configured slots are not available job slots. Labels are not recorded in local descriptors; inspect them in the
        provider.
      </p>
  ...
  private pageDisabled(): boolean {
    return this.busy || this.stale || this.blocked || !this.lifetime;
  }
  private controlsLocked(): boolean {
    return this.busy || !this.lifetime;
  }
  private statusNoticeView() {
    return this.notice ? html`<p role="status" class="settings-notice">${this.notice}</p>` : '';
  }
  ...
  private confirmationView(disabled: boolean) {
    ...
    return html` <section class="settings-confirmation" aria-label="Confirm runner operation">
      <h2>${this.pending.action} ${this.pending.id}</h2>
      <p>${effects[this.pending.action]}</p>
      <form
        @submit=${(event: SubmitEvent) => this.confirm(event)}
        @keydown=${(event: KeyboardEvent) => this.confirmationKey(event)}
      >
        ${this.pending.action === 'remove' ? html`<label>Exact runner ID<input name="confirm_id" autocomplete="off" required /></label>` : ''}
        <div class="settings-actions">
          <button type="button" data-runner-cancel @click=${() => void this.cancelConfirmation()}>Cancel</button
          ><button ?disabled=${disabled}>Confirm ${this.pending.action}</button>
        </div>
      </form>
    </section>`;
  }
  protected render() {
    const disabled = this.pageDisabled();
    return html` <div class="settings-actions">
      ...
      ${this.confirmationView(disabled)}
      <section aria-labelledby="runner-provider-title">
        <h2 id="runner-provider-title">Provider access</h2>
        <p>
          <a href=${location.origin + '/admin/actions/runners'}>Forgejo Actions administration</a> manages native runner
