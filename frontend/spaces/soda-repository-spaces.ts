...
export function mountRepositorySpaces(mount: HTMLElement, actor: string, repository: string, session?: Session) {
  ...
  controls.configure({expectedUserId: actor, repositoryId: repository, page: true, settings: true, session});
