...
export function connectionSuppressed(): boolean {
  try {
    return retired || localStorage.getItem(suppressionKey) !== null;
  } catch {
    return retired;
  }
}
...
window.addEventListener('pageshow', () => {
  if (connectionSuppressed()) retire();
});
...
function suppress() {
  retire();
  try {
    localStorage.setItem(suppressionKey, String(Date.now()));
  } catch {
    /* UI signal only. */
  }
}
...
async function readActorSession(actor: string, signal: AbortSignal) {
...
async function finishPendingCancel(headers: Record<string, string>, signal: AbortSignal) {
  ...
  if (pending.status === 200) {
    ...
    if (ended.status !== 204) {
      ...
      throw Error('Cancellation unconfirmed');
    }
    return true;
  }
  if (pending.status !== 204) {
    await pending.body?.cancel();
    throw Error('Cancellation unavailable');
  }
  return false;
}

async function endActorSession(session: Session, actor: string, signal: AbortSignal) {
  const ended = await request(
    '/api/session/logout',
    {'Content-Type': 'application/json', 'X-CSRF-Token': session.csrf_token, 'X-Soda-Expected-User-ID': actor},
    true,
    signal
  );
  if (ended.status !== 204) {
    await ended.body?.cancel();
    throw Error('Logout unconfirmed');
  }
}
...
async function cancel(actor: string, signal: AbortSignal) {
  ...
  const session = await readActorSession(actor, signal);
  const headers = {'X-Soda-Logout': '1', 'X-Soda-Expected-User-ID': actor};
  if (await finishPendingCancel(headers, signal)) return;
  if (!session) return;
  await endActorSession(session, actor, signal);
}
...
function notice(message: string, actor: string, link: HTMLAnchorElement | null, failed: boolean) {
  ...
  box.id = 'soda-signout-status';
  box.className = 'ui warning message';
  ...
  if (failed) {
    ...
    retry.addEventListener('click', () => void signOut(actor));
    box.append(retry);
  }
  if (link) {
    const escape = document.createElement('button');
    escape.type = 'button';
    escape.className = 'ui button';
    escape.textContent = failed ? 'Sign out of Forgejo only' : 'Retry Forgejo sign-out';
    ...
    back.href = sub + '/';
    back.textContent = 'Continue to Forgejo to sign out';
    box.append(back);
  }
...
function logoutActionLink(target: Element | null) {
  const link = target?.matches('a.link-action') ? target : null;
  return link instanceof HTMLAnchorElement ? link : null;
}

function captureLogoutClick(event: MouseEvent) {
  const target = clickAnchor(event);
  if (sodaLoginLink(target)) allowExplicitConnection();
  const link = logoutActionLink(target);
  if (!link || link !== nativeLogout()) return;
  if (replay === link) {
    replay = null;
    return;
  }
...
document.addEventListener('click', captureLogoutClick, true);

const pageDestinations = ['spaces', 'runners', 'tailnet', 'repository-spaces'] as const;

function admitPageContext(actor: string, destination: string, repository: string) {
  if (
    !id(actor) ||
    !pageDestinations.includes(destination as (typeof pageDestinations)[number]) ||
    (destination === 'repository-spaces' ? !id(repository) : repository !== '')
  )
    throw Error('Invalid page context');
}

function markConnectAttempt(key: string, retry: boolean) {
  try {
...
function assignSodaLogin(destination: string, actor: string, repository: string) {
...
function entryStillCurrent(isCurrent: () => boolean) {
  return isCurrent() && !connectionSuppressed();
}

async function adoptPageSession(response: Response, actor: string, key: string, isCurrent: () => boolean) {
  const session = sessionResponse(await readSodaJSON(response), location.origin);
  if (!entryStillCurrent(isCurrent) || session.user.id !== actor) throw Error('Entry or Forgejo account changed.');
  try {
...
async function missingPageSession(response: Response, isCurrent: () => boolean, restoring: boolean) {
  const missing = response.status === 401;
  await response.body?.cancel();
  if (!entryStillCurrent(isCurrent)) return false;
...
async function claimPageSession(
  actor: string,
  key: string,
  isCurrent: () => boolean,
  restoring: boolean
  ...
  if (!entryStillCurrent(isCurrent)) {
    await response.body?.cancel();
    throw Error('Entry retired.');
  }
  if (response.ok) return adoptPageSession(response, actor, key, isCurrent);
  return missingPageSession(response, isCurrent, restoring);
}

function clearConnectMarker(key: string) {
  try {
    sessionStorage.removeItem(key);
  } catch {
...
function rejectIncompleteConnect(retry: boolean) {
  if (!retry && new URL(location.href).searchParams.has('soda-connect'))
    throw Error('Forgejo connection did not complete.');
}
...
export async function connectPage(
  ...
  admitPageContext(actor, destination, repository);
  const key = 'soda-entry:' + destination + ':' + repository + ':' + actor;
  if (!isCurrent()) return false;
  if (retry) {
    allowExplicitConnection();
    clearConnectMarker(key);
  }
  if (connectionSuppressed()) throw Error('Sign-out requested. Retry connection explicitly.');
  rejectIncompleteConnect(retry);
  const claimed = await claimPageSession(actor, key, isCurrent, restoring);
  if (claimed !== undefined) return claimed;
  markConnectAttempt(key, retry);
  assignSodaLogin(destination, actor, repository);
  return false;
}
