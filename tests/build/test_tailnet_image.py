...
class TailnetImage(unittest.TestCase):
    ...
    def test_metadata_refuses_wrong_or_malformed_binary_versions(self):
        ...
        clis = {'/usr/local/bin/tailscale': json.dumps({'short': version}), '/usr/local/bin/tailscaled': version + '-tscommit\n'}
        check(clis, version)
        for binary, output in [('/usr/local/bin/tailscale', '{}'), ('/usr/local/bin/tailscale', '[]'),
                               ('/usr/local/bin/tailscale', '{"short":"1.0.0"}'),
                               ('/usr/local/bin/tailscaled', ''), ('/usr/local/bin/tailscaled', '1.0.0')]:
