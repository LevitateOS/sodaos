...
test('reservation permission admits only the selected name and measured bounded geometry', () => {
  const body = {cols:194, rows:41, name:'selected'}; assert(matchesTerminalReservation(JSON.stringify(body), 'selected'));
  for (const changed of [{...body, name:'other'}, {...body, cols:true}, {...body, rows:301}, {...body, command:'id'}, null]) assert(!matchesTerminalReservation(JSON.stringify(changed), 'selected'));
...
for (const actorIndex of [0, 1]) test(`matrix and page/drawer navigation actor ${actorIndex} preserve exact sessions against a synthetic peer`, {timeout: 60000}, async () => {
  ...
  const server = Bun.serve<Wire>({hostname: '127.0.0.1', port: 0, tls: {key: Bun.file(tls + '/key.pem'), cert: Bun.file(tls + '/cert.pem')},
    async fetch(req, server) {
      ...
      if (route.startsWith('/-/soda/api/')) {
        ...
        if (route.endsWith('/spaces')) return Response.json({items: spaces.map(space => ({...space, terminals: space.terminals.filter(t => t.state !== 'ended')})), complete: true});
        ...
        if (route.endsWith('/terminal-sessions') && req.method === 'POST') {
          const body = object(await req.json());
          assert(matchesTerminalReservation(JSON.stringify(body), String(body.name)));
          if (allowed) {assert.equal(route + ':' + JSON.stringify({name:body.name}), allowed); allowed = '';}
          else {assert.equal(space, spaces[0]); assert(body.name === 'From Spaces' || body.name === 'From drawer');}
          const id = (++ids).toString(16).padStart(32, '0'); reservations.set(id, space.environment.id);
        ...
        if (route.includes('/terminal-sessions/')) {
          ...
          if (req.method === 'POST') {
            ...
            assert.equal(body.action, 'end'); terminal.state = 'ended'; terminal.ready = false; terminal.attached = false; writers.get(terminal.id)?.close();
          }
      ...
      message(ws, raw) {
        ...
        if (frame.action === 'create') {
          assert(typeof frame.id === 'string' && typeof frame.name === 'string' && !terminal && reservations.get(frame.id) === space.environment.id); reservations.delete(frame.id);
          terminal = {id: frame.id, environment_id: space.environment.id, repository_id: space.environment.repository_id, user_id: actor, login, name: frame.name, created_at: now, ready: true, attached: false, state: 'ready'} satisfies TerminalMetadata;
  ...
  try {
    ...
    await exerciseWorkspaceMatrix(page, request, actorIndex, (session, action) => {assert.equal(allowed, ''); allowed = `/-/soda/api/environments/${session.environment}/terminal-sessions` + (action === 'end' ? `/${session.id}:` + JSON.stringify({action}) : ':' + JSON.stringify({name:session.name}));}, {
    ...
    assert.deepEqual(writes, ['end', 'end', 'end', 'end', 'end', 'end']); assert.equal(allowed, ''); assert.deepEqual(errors, []);
    ...
    assert.equal(await page.evaluate(actor => sessionStorage.getItem('soda-spaces:v3:' + actor), actor), layout);
    ...
    assert.deepEqual(writes, ['end', 'end', 'end', 'end', 'end', 'end']);
