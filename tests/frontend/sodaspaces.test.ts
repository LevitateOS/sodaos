...
async function fixture(t: TestContext, options: {hash?: string; context?: Record<string,string>; noRow?: boolean; duplicate?: boolean; fetch?: (url: string, init: RequestInit) => Promise<Response>; mount?: Parameters<typeof mountDrawer>[1]} = {}) {
...
test('late fetch after document departure cannot repaint or dispatch another request', async t => {
  ...
  assert.equal(f.calls.length, 1); assert(f.button('Reload repository page')); assert(!f.doc.getElementById('sodaspaces-create'));
});
