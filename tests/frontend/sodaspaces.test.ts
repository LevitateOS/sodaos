...
async function fixture(
  ...
  options: {
    width?: number;
    saved?: {repositoryId: string; open: boolean; width?: number};
    hash?: string;
    context?: Record<string, string>;
    noRow?: boolean;
    duplicate?: boolean;
    mount?: Parameters<typeof mountDrawer>[1];
  } = {}
) {
  const dom = new JSDOM(
    `<div class="repo-header"><div class="repo-buttons"><button id="native">Native</button></div></div><form><input id="native-edit" value="dirty"></form>${footer}`,
    {url: `https://forge.test/alice/repo${options.hash || ''}`, pretendToBeVisual: true}
  );
  const w = dom.window,
    doc = w.document;
  Object.defineProperty(w, 'innerWidth', {value: options.width || 1440, writable: true});
  const $ = (n: string) => {
    const element = doc.getElementById('sodaspaces-' + n);
    assert(element);
    return element;
  };
...
for (const options of [
  {noRow: true},
  {duplicate: true},
  {context: {subUrl: '/unsupported'}},
  {context: {userId: '0'}},
  {context: {repositoryId: '07'}},
  {context: {signed: 'false', userId: '1'}},
]) {
...
test('only exact OAuth fragment opens, and large IDs stay strings', async (t) => {
  ...
  const f = await fixture(t, {
    hash: '#sodaspaces',
    context: {userId: '9007199254740993'},
    mount: (_root, context) => {
      contexts.push(context);
      return {refresh() {}, dispose() {}};
    },
  });
  assert(!f.$('drawer').hidden);
  assert.equal(contexts[0]?.expectedUserId, '9007199254740993');
});
for (const event of ['pagehide', 'bfcache']) {
  test(event + ' detaches and fresh-mounts only after document restoration', async (t) => {
    ...
    assert(![...f.$('content').querySelectorAll('button')].some((b) => b.textContent === 'Reload repository page'));
...
test('non-modal native interaction, blur and hide/reopen preserve one mounted component', async (t) => {
  ...
  await f.open();
  assert.equal(f.$('drawer').tagName, 'ASIDE');
  assert(!f.doc.querySelector('[inert], [aria-modal=true]'));
  f.doc.getElementById('native-edit')?.focus();
  const edit = f.doc.querySelector<HTMLInputElement>('#native-edit');
  assert(edit);
  edit.value = 'still editable';
  f.doc.getElementById('native')?.click();
  f.$('drawer').click();
  f.w.dispatchEvent(new f.w.Event('blur'));
  f.doc.dispatchEvent(new f.w.Event('visibilitychange'));
  assert(!f.$('drawer').hidden);
  assert(f.doc.body.classList.contains('sodaspaces-open'));
  f.$('close').click();
  assert(f.$('drawer').hidden);
  assert(!f.doc.body.classList.contains('sodaspaces-open'));
  await f.open();
...
test('native repository context cannot replace the shared working set', async (t) => {
  ...
  let disposed = 0;
  const f = await fixture(t, {
    context: {repositoryId: '8'},
    saved: {repositoryId: '7', open: true, width: 55},
    mount: (_root, context) => {
      contexts.push(context);
      return {
        refresh() {},
  ...
  assert(!f.$('drawer').hidden);
  assert.equal(contexts[0]?.repositoryId, '8');
  assert.equal(f.doc.body.style.getPropertyValue('--soda-space-width'), '55vw');
...
test('signed non-repository page resumes only a saved workspace locator', async (t) => {
  ...
  const f = await fixture(t, {
    ...
    saved: {repositoryId: '7', open: true},
    mount: (_root, context) => {
      contexts.push(context);
      return {refresh() {}, dispose() {}};
    },
  });
  assert.equal(contexts[0]?.repositoryId, '7');
  assert(!f.$('drawer').hidden);
...
test('measured native/terminal minimums clamp actual widths without forcing a breakpoint split', () => {
  ...
  const narrow = workspaceWidths(1024, 532, 65);
  assert.equal(narrow.compact, false);
  assert.equal(narrow.actual, 53.125);
...
test('compact visibility is document-local and never disposes live owners', async (t) => {
  ...
  const controls = f.$('surfaces').querySelectorAll('button');
  controls[1]?.click();
  controls[0]?.click();
  assert.equal(disposed, 0);
  assert(visible.includes(true));
  const stored = JSON.parse(f.w.sessionStorage.getItem('soda-workspace:1') || '{}');
  assert.deepEqual(stored, {repositoryId: '7', open: true, width: 55});
  f.w.dispatchEvent(new f.w.PageTransitionEvent('pageshow', {persisted: true}));
  assert.equal(f.$('drawer').getAttribute('aria-hidden'), 'true');
});
test('framed Forgejo documents do not mount a nested drawer', () => {
  assert.equal(mountDrawer({defaultView: {frameElement: {}}} as unknown as Document), undefined);
});
