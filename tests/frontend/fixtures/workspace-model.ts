...
export function createWorkspaceModel(origin: string, actor = '1', repository?: {id: string; owner: string; name: string}, firstUse = false, storage?: Pick<Storage, 'getItem' | 'setItem'>) {
  ...
  if (repository) {
    ...
    for (const space of spaces) for (const terminal of space.terminals) {
      const saved = storage?.getItem('fixture-terminal:' + terminal.id);
    ...
    if (path.includes('/terminal-sessions/')) {
      ...
      if (terminal && body) {
        ...
        if (body.action === 'end') {
          ...
          if (repository) storage?.setItem('fixture-terminal:' + terminal.id, terminal.state);
        }
      }
  ...
  class Socket {
    ...
    constructor(readonly url: string | URL) {sockets.push(this); setTimeout(() => {if (!this.closed) {this.readyState = 1; this.onopen?.();}}, 0);}
    send(text: string) {
