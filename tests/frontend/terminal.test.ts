...
function fixture(t: TestContext, options: {existing?: {id: string; login: string; repository_id: string; state?: string}; saved?: string; slow?: boolean; fetch?: (url: string, init: RequestInit) => Promise<Response>} = {}) {
  ...
  Object.defineProperty(w, 'fetch', {value: async (url: string, init: RequestInit) => {
    ...
    if (url.endsWith('/terminal-session')) {
      if (init.method === 'POST') {
        ...
        assert(input && typeof input === 'object' && 'action' in input);
  ...
  class FakeSocket {
    ...
    message(value: unknown) {
      ...
      assert(this.onmessage); this.onmessage({data: JSON.stringify(value)});
    }
  }
...
async function ready(f: ReturnType<typeof fixture>) {
  ...
  assert.equal(f.sockets.length, 1); const socket = f.sockets[0]; assert(socket); socket.open(); socket.message({type: 'session', id: 'a'.repeat(32)}); socket.message({type: 'ready'}); return socket;
}
...
test('End is a separate authenticated action; keyboard escape and focus stay local', async t => {
  ...
  assert(action && typeof action.init.body === 'string');
...
test('known locator restores attach-only without credentials in storage', async t => {
  ...
  assert.equal(f.sockets.length, 1); const socket = f.sockets[0]; assert(socket); socket.open();
  assert.equal(socket.sent[0]?.action, 'attach'); assert.equal(socket.sent[0]?.id, id);
