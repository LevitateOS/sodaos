...
def entry(path, contents=True):
    ...
    if p.is_symlink():
        ...
        if s.st_size > 512 * 1024 * 1024:
            raise RuntimeError('Snapshot file too large')
        h = hashlib.sha256()
        with p.open('rb') as f:
        ...
        raise RuntimeError('Unsupported snapshot file')
    return result
...
def main():
    ...
    paths = [
        '/etc/passwd',
        '/etc/group',
        '/etc/ssh/sshd_config.d/10-soda.conf',
        '/etc/ssh/ssh_host_ed25519_key.pub',
        '/etc/mise/config.toml',
        '/etc/containers/containers.conf',
        '/etc/containers/storage.conf',
        '/etc/systemd/system/soda-podman.service',
        '/usr/libexec/soda/project-init',
        '/usr/libexec/soda/project-account',
        '/etc/sudoers.d/soda-project',
    ]
    optional = Path('/etc/systemd/system/soda-podman.socket')
    if optional.exists():
        paths.append(str(optional))
    ...
    for login in ['u08-alice-8417', 'u08-bob-8417']:
        ...
        if not home.exists():
            continue
        data['people'][login] = {
            'identity': command('id', login),
            'home': entry(home),
            'shared': entry(home / 'shared'),
        }
        ...
        if checkout.exists():
            ...
            if len(files) > 10000:
                raise RuntimeError('Checkout snapshot exceeded bound')
            for file in sorted(files):
                if '.git' in file.relative_to(checkout).parts:
                    continue
                if file.is_file() or file.is_symlink():
                    paths.append(str(file))
            data['git'][login] = {
                'head': command('runuser', '-u', login, '--', 'git', '-C', str(checkout), 'rev-parse', 'HEAD'),
        ...
        for private in ['.ssh/u08-personal-git/identity', '.config/soda-u08-workload/database-password']:
            f = home / private
            if f.exists():
                data['files'][str(f)] = entry(f, contents=False)
    ...
    if os.environ['SODA_EXPECT_WORKLOADS'] == '1':
        ...
        databases = [
            name
            for name in command(*podman, 'ps', '--format', '{{.Names}}').splitlines()
            if name in ['u08-projectnet-database', 'workload_database_1']
        ]
        ...
        for name in databases:
            ...
            data['database'][name] = command(
                ...
                env={**os.environ, 'PGPASSFILE': str(passfile), 'PGCONNECT_TIMEOUT': '5'},
            )
