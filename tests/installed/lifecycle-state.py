...
def main():
    ...
    if mode == 'compare':
        ...
        print(
            'All declared project/account/Git/tool/workload/database state matches; boot identity compared separately.'
        )
        return
...
'''
    ...
    for identifier, login, workloads in entries:
        ...
        identity = (
            run([vm, 'ssh', 'podman inspect --format "{{.Id}} {{.Image}} {{.Name}}" soda-' + identifier])
            .decode()
            .strip()
        )
        network = json.loads(
            run([vm, 'ssh', 'podman inspect --format "{{json .NetworkSettings.Networks}}" soda-' + identifier])
        )
        ...
        state = json.loads(
            run(
                [
                    vm,
                    'ssh',
                    'podman exec -i --env SODA_PROJECT_IP='
                    + ip
                    + ' --env SODA_EXPECT_WORKLOADS='
                    + ('1' if workloads else '0')
                    + ' soda-'
                    + identifier
                    + ' python3 -',
                ],
    ...
    print(
        'Complete bounded snapshot captured for three projects and Soda associations; no secret/private-key contents exported.'
    )
