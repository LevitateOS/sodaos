...
def main():
    ...
    target = (
        json.loads((root / 'target.json').read_text())
        if (root / 'target.json').exists()
        else {'ip': '10.89.0.2', 'network_mode': 'project namespace (nested host mode)'}
    )
    ip = target['ip']
    ...
    def remote(who, command, data=None):
        result = subprocess.run(
            ['ssh', '-F', str(config), 'u08-' + who + '-8417@' + ip, command],
            input=data,
        ...
        if result.returncode:
            raise RuntimeError('Native member operation failed')
    ...
    for who in ('alice', 'bob'):
        remote(
            who,
            f'umask 077; mkdir -p "$HOME/.config"; mkdir -m700 "$HOME/.config/u08-db-client-{run}"; cat > "$HOME/.config/u08-db-client-{run}/pgpass"',
            record,
        )
    del record
    parameters = dict(
        host=ip, port=5432, dbname='soda_example', user='developer', passfile=str(passfile), connect_timeout=10
    )
    ...
    with psycopg.connect(**parameters) as database:
        assert database.execute('SELECT value FROM soda_u08_probe WHERE run_id=%s', (run,)).fetchone() == (
            'client-committed',
        )
    for who in ('alice', 'bob'):
    ...
    for who in ('alice', 'bob'):
        assert (
            remote(who, 'curl --noproxy "*" --fail --silent --show-error --max-time 10 http://' + ip + ':8000/')
            == content
        )
        ...
        json.dumps(
            {
                ...
                'default_nested_bridge_verified': target['network_mode'] == 'bridge',
            },
            indent=2,
        )
    )
