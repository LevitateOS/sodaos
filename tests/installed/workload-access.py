...
def main():
    ...
    target = json.loads((root / 'target.json').read_text()) if (root / 'target.json').exists() else {'ip': '10.89.0.2', 'network_mode': 'project namespace (nested host mode)'}
    ip = target['ip']; assert ipaddress.ip_address(ip) in ipaddress.ip_network('10.89.0.0/24')
    run = uuid.uuid4().hex
    ...
    def remote(who, command, data=None):
        result = subprocess.run(['ssh', '-F', str(config), 'u08-' + who + '-8417@' + ip, command],
    ...
    del record
    parameters = dict(host=ip, port=5432, dbname='soda_example', user='developer', passfile=str(passfile), connect_timeout=10)
    ...
    for who in ('alice', 'bob'):
        ...
        prefix = f'PGPASSFILE="$HOME/.config/u08-db-client-{run}/pgpass" psql -X -w -h {ip} -p 5432 -U developer -d soda_example -At -v ON_ERROR_STOP=1'
    ...
    with opener.open('http://' + ip + ':8000/', timeout=10) as response:
        assert response.status == 200 and response.read(1048576) == content
    for who in ('alice', 'bob'):
        assert remote(who, 'curl --noproxy "*" --fail --silent --show-error --max-time 10 http://' + ip + ':8000/') == content
    (output / 'results.json').write_text(json.dumps({'run_id': run, 'network_mode': target['network_mode'],
        'http_bind_edit_client_and_members': True, 'postgres_client_and_members': True, 'committed_value': 'bob-committed',
        'restart_persistence_verified': False, 'default_nested_bridge_verified': target['network_mode'] == 'bridge'}, indent=2))
    print('Client and Alice/Bob verified live HTTP bind-mount edit and committed PostgreSQL read/write/readback.')
    print('Network mode: ' + target['network_mode'] + '; restart/reboot persistence is a separate check.')
