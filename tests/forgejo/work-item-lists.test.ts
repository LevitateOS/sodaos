...
test('work items share appearance across native callers and keep usable metadata', { skip: !origin }, async () => {
  ...
  try {
    ...
    for (const family of ['issues', 'pulls', 'milestones']) {
      ...
      samples[family] = await page.locator(selector).evaluateAll(es => [es.find(e => e.querySelector('.label')) || es[0], es.at(-1)].map(e => {if(!e) throw Error('Missing native work-item sample'); return e.outerHTML;}).join(''));
    }
    ...
    for (const theme of ['light', 'dark']) for (const family of ['issues', 'pulls']) {
      const nativeTheme: Response = await fetch(`${origin}/assets/css/theme-forgejo-${theme}.css`);
      ...
      for (const context of ['soda-repo-work-items', 'soda-page', 'soda-page soda-pulls', 'soda-notifications']) {
        ...
        for (const width of [1440, 1024, 700, 601, 600, 390, 320]) {
          ...
          const rows = await page.locator('#issue-list > .flex-item, .soda-milestone-row').evaluateAll(es => es.map(e => {
            ...
            const titleElement=e.querySelector('.flex-item-title,.soda-milestone-name'), linkElement=e.querySelector('.issue-title,.soda-milestone-name > a');
            if(!titleElement || !linkElement) throw Error('Missing work-item title or link');
