...
test('refined page families fit narrow screens and keep content readable',{skip:!enabled},async()=>{
 ...
 try {
  ...
  for(const theme of ['light','dark']) for(const width of [320,390,768,1440]) {
   ...
   for(const name of ['migration','refinement','signin','home']) {
    const response=await page.goto(`${origin}/${name}-${theme}.html`);assert.equal(response?.status(),200);
    ...
    if(name==='migration') {
     ...
     assert(contrast(colors.bg,colors.fg)>=4.5,'Git source description must stay readable on hover');
    ...
    if(name==='refinement') {
     ...
     assert.equal(formColumns,width<761?1:2);
  ...
  await page.goto(`${origin}/refinement-light.html`);
  await page.setViewportSize({width:1440,height:1000});
  await page.locator('main').evaluate(el=>el.insertAdjacentHTML('beforeend','<div class="soda-account-details--profile"><div class="soda-profile-portrait"><img id="editor-portrait" alt="Fixture avatar" src="/assets/soda/source/soda-symbol-brutalist.svg"></div></div><div class="soda-profile-card-context"><div class="soda-profile-portrait"><img id="public-portrait" alt="Fixture avatar" src="/assets/soda/source/soda-symbol-brutalist.svg"></div></div>'));
