...
test('presentation inventory covers every production override and its local callers', async () => {
  const { entries, upstream } = (await import('./inventory.json')).default;
  assert.equal(upstream, '15.0.7');
  assert.deepEqual(entries.map(e => e.template).sort(), await templates());
  const sources = new Map(await Promise.all(entries.map(async e => [e.template, await readFile(new URL(e.template, root), 'utf8')] as const)));
  for (const entry of entries) {
    ...
    const source = sources.get(entry.template); assert(source !== undefined);
    const beforeRoles = source.replace(/ soda-p-(editor-container|form-host|form|title|heading|section|gap|toolbar)\b/g, '');
    ...
    for (const role of entry.roles) assert(source.includes(role), `${entry.template}: missing ${role}`);
  }
});
