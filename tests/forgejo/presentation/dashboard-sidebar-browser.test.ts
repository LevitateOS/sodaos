...
test('dashboard repository browser supports both themes and terminal-sized panes', {skip:process.env.SODA_FORGEJO_SIDEBAR_REVIEW!=='1'},async()=>{
 ...
 try{
  ...
  for(const theme of ['light','dark'])for(const width of [320,640,720,800,960,1199,1440]){
   ...
   await page.waitForFunction(theme=>getComputedStyle(document.querySelector('#dashboard-repo-list .tabs-with-labels > .active')!).color===(theme==='light'?'rgb(255, 255, 255)':'rgb(16, 16, 16)'),theme);
   ...
   assert.equal(await active.evaluate(e=>getComputedStyle(e).color),theme==='light'?'rgb(255, 255, 255)':'rgb(16, 16, 16)');
