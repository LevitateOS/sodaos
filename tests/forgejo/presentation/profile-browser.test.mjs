  ...
  for(const theme of ['light','dark']) for(const width of [1440,900,701,700,390,320]) {
   await t.test(`${theme} ${width}`,async()=>{
    ...
    for(const route of ['/alice','/alice?tab=activity','/alice?tab=followers','/alice?tab=following','/alice?tab=stars','/alice/-/projects','/alice/-/packages']) {
     ...
     assert.equal(response.status(),200);assert.equal(page.url(),`http://localhost:3300${route}`);
     ...
     const layout=await page.locator('.soda-profile-masthead').evaluate(e=>({display:getComputedStyle(e).display,avatar:e.querySelector('img').getBoundingClientRect().width,overflow:document.documentElement.scrollWidth>innerWidth}));
    ...
    await menu.focus();await page.keyboard.press('Enter');assert(await page.locator('.soda-profile-actions details').evaluate(e=>e.open));
