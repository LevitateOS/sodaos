...
const gallery=theme=>new URL(`../../../.artifacts/forgejo-presentation/repository-settings-${theme}.html`,import.meta.url).href;
  ...
  for(const theme of ['light','dark']) for(const width of [1440,1000,900,899,768,390,320]) {
   await t.test(`${theme} at ${width}px`,async()=>{
    ...
    const layout=await page.evaluate(()=>{
     ...
     return {overflow:document.documentElement.scrollWidth>innerWidth,padding:getComputedStyle(container).paddingInlineStart,inset:getComputedStyle(content).paddingInlineStart,title:getComputedStyle(document.querySelector('h1')).fontSize,left:document.querySelector('h1').getBoundingClientRect().left};
    });
    ...
    for(const button of await page.locator('.repo-setting-content button').all()) {
     if(await button.isVisible()) assert((await button.boundingBox()).height>=44,'shared button height');
    }
