...
test('notification preview uses native HTMX with stable accessible lifecycle', { skip: !origin }, async () => {
  ...
  try {
    ...
    for (const next of ['empty', 'error', 'unexpected']) {
      ...
      assert(!await panel.textContent().then(text => text.includes('Private error body') || text.includes('Unexpected login')));
      if (next !== 'empty') {
        mode = 'populated';
    ...
    assert(!await panel.textContent().then(text => text.includes('Stale response')));
    ...
    for (const fallback of ['no-js', 'no-htmx', 'no-popover']) {
      ...
      await fallbackPage.route('**/explore/repos', route => route.fulfill({ contentType: 'text/html', body: fixtureHTML }));
      ...
      if (fallback === 'no-popover') {
        await fallbackPage.addInitScript(() => { HTMLElement.prototype.showPopover = undefined; });
      }
