...
func (s *Store) BeginOAuth(ctx context.Context, state string, login OAuthLogin, session, previous string) error {
	if (login.SpacesReturn && login.RepositoryID != 0) || (login.SettingsReturn != "" && (login.SettingsReturn != "runners" || login.SpacesReturn || login.RepositoryID != 0)) {
	...
	_, err = tx.ExecContext(ctx, `INSERT INTO oauth(state,verifier,expires,repository_id,expected_user_id,context_id,spaces_return,settings_return) VALUES(?,?,?,?,?,?,?,?)`, hash(state), login.Verifier, expires, login.RepositoryID, login.ExpectedUserID, id, login.SpacesReturn, login.SettingsReturn)
	if err != nil {
		return err
	}
	return tx.Commit()
