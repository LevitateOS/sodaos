...
func TestOAuthLoginBindsBoundedContextToPKCEState(t *testing.T) {
	...
	for _, query := range []string{
		"destination=repository-spaces", "destination=repository-spaces&repository_id=0", "destination=repository-spaces&repository_id=7&repository_id=8", "destination=repository-spaces&repository_id=7&return_to=https://evil.test", "destination=runners&repository_id=7", "destination=runners&destination=spaces", "destination=", "destination=other", "destination=spaces&destination=spaces", "destination=spaces&repository_id=7", "destination=spaces&repository_id=", "destination=https://elsewhere.test",
...
func TestOAuthRepositoryReturnUsesOnlyStoredIDsAndActingGrant(t *testing.T) {
	for _, tc := range []struct {
		...
		{"fixed repository settings", "destination=repository-spaces&repository_id=42&expected_user_id=1", "read:user read:repository", 0, "", "/-/soda/repositories/42/settings/spaces", 3},
