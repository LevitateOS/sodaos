...
(async () => {
  ...
  try {
    ...
    for (const sample of [0, 1, 8, 23, 59, 99]) {
      const hash = new Bun.CryptoHasher('md5').update(`soda-avatar-preview-${String(sample).padStart(3, '0')}`).digest('hex');
      ...
      await page.locator('#actual').evaluate((image, url) => { if (!(image instanceof HTMLImageElement)) throw Error('Missing avatar image'); image.src = url; }, `${origin}/reference/${hash}`);
      await page.locator('#actual').evaluate(image => {if (!(image instanceof HTMLImageElement)) throw Error('Missing avatar image'); return image.decode();});
