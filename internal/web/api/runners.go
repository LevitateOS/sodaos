...
func (s *API) apiRunners(w http.ResponseWriter, r *http.Request, v store.Session) {
...
func (s *API) apiRunnerAction(w http.ResponseWriter, r *http.Request, v store.Session) {
	ctx, cancel := context.WithTimeout(r.Context(), 3*time.Minute)
	defer cancel()
	r = r.WithContext(ctx)
	if !s.authorizeOperator(w, r, v) {
