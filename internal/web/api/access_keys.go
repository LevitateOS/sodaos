...
func (s *API) applyAccessKeyMutation(w http.ResponseWriter, r *http.Request, v store.Session, fingerprints, keys []string, in *project.AccessKeys) bool {
	var input struct {
	...
	if !validAccessKeyConfirmation(input.Revision, input.SavedFingerprints, keys, input.ConfirmEmpty) {
		auth.JSONError(w, 400, "invalid_key_confirmation", "Review this project's key changes and explicitly confirm removal of the last key.")
		return false
	}
	if !slices.Equal(input.SavedFingerprints, fingerprints) {
		auth.JSONError(w, 409, "saved_keys_changed", "Saved keys changed; review again before applying.")
		return false
	}
	in.Apply = true
...
func nativeAccessFingerprints(w http.ResponseWriter, publics []string) ([]string, bool) {
	...
	for _, public := range publics {
		...
		if err != nil {
			auth.JSONError(w, 502, "native_unavailable", "Invalid native key observation.")
			return nil, false
		}
...
func (s *API) apiAccessKeys(w http.ResponseWriter, r *http.Request, v store.Session) {
	...
	in := project.AccessKeys{Project: p.ID, Login: login, Identity: v.User.ID}
	saved, err := s.Store.Keys(r.Context(), v.User.ID)
	if err != nil {
		auth.JSONError(w, 503, "store_unavailable", "Could not read saved keys.")
		return
	}
	...
	if err != nil {
		auth.JSONError(w, 502, "native_outcome_unconfirmed", "Key state/update was not confirmed. Refresh and inspect before any further action; saved-key changes alone do not revoke SSH.")
		return
	}
