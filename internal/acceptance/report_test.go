...
func TestHandoffPreservesMissingAndFailedScopes(t *testing.T) {
	...
	if err = Handoff(filepath.Join(t.TempDir(), "wrong.md"), "aarch64", revision, []string{record}); err == nil {
		t.Fatal("mixed sibling evidence")
	}
	if err = os.WriteFile(filepath.Join(e.Path(), "check.stdout"), []byte("changed"), 0600); err != nil {
		t.Fatal(err)
	}
	if err = Handoff(filepath.Join(t.TempDir(), "changed.md"), "x86_64", revision, []string{record}); err == nil {
