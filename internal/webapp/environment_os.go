...
func rejectOSQuery(w http.ResponseWriter, r *http.Request) bool {
	if r.URL.RawQuery != "" || r.URL.ForceQuery {
		webauth.JSONError(w, 400, "invalid_request", "OS observations accept no query parameters.")
		return true
	}
	return false
}
...
func (s *API) apiEnvironmentOS(w http.ResponseWriter, r *http.Request, v store.Session) {
	...
	if err != nil || environmentProfileMismatch(p, observed.Environment) {
		webauth.JSONError(w, 503, "os_unavailable", "Native OS observation unavailable. Nothing was started or repaired.")
		return
	}
	if !s.confirmOSSession(w, r, ctx, v) {
		return
	}
	webauth.JSONResponse(w, 200, observed)
}
