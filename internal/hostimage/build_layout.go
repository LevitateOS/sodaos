...
func validStageLayout(archives, destination string, p appliancerelease.Payload, run nativebuild.BuildExec) bool {
	return p.Format == 3 && run != nil && filepath.IsAbs(archives) && filepath.IsAbs(destination) && !strings.ContainsAny(archives+destination, ":\r\n")
}

func verifyArchiveDigests(archives string, p appliancerelease.Payload) error {
...
func copyStagedArchives(archives, destination string, p appliancerelease.Payload, run nativebuild.BuildExec) error {
	for _, name := range appliancerelease.Names {
