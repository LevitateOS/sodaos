...
func InitState(path string, t Trust) error {
	...
	if e := build.PrivateDestination(path); e != nil {
		return e
	}
	s := EmptyState()
...
func checkVerifiedImageMetadata(config []byte, arch string) error {
	...
	want, _ := build.OCIArchitecture(arch)
...
func buildExpectedReleaseConfigs(candidate Candidate, p Payload) map[string]string {
...
func admitFetchRequest(t Trust, name, arch string) error {
	...
	_, e := build.OCIArchitecture(arch)
	return e
}
...
func Fetch(ctx context.Context, r Runner, t Trust, name, arch, statePath, out string, now time.Time) error {
	...
	if e = build.FreshDirectory(out); e != nil {
		return e
	}
