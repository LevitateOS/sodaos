...
func putDocumentBlob(blobs string, b []byte, media string) (descriptor, error) {
	h := Hash(b)
	e := build.WriteNew(filepath.Join(blobs, strings.TrimPrefix(h, "sha256:")), b, 0o600)
...
func writeDocumentIndex(path string, md descriptor) error {
	...
	if e := build.WriteNew(filepath.Join(path, "index.json"), index, 0o600); e != nil {
		return e
	}
	return build.WriteNew(filepath.Join(path, "oci-layout"), []byte(`{"imageLayoutVersion":"1.0.0"}`), 0o600)
}
...
func WriteDocument(path string, value any) (string, error) {
	...
	if e = build.FreshDirectory(path); e != nil {
		return "", e
	}
