...
func testMediaBytes(t *testing.T, p Payload, c Candidate) []byte {
	t.Helper()
	media := MediaBinding{
...
func testRelease(t *testing.T, tr Trust) Release {
	p := Payload{Format: 3, CoreOS: "44.20260817.3.2", Revision: strings.Repeat("a", 40), Architecture: "x86_64", Schema: 10, RepositoryPrefix: tr.Prefix, Base: "quay.io/fedora/fedora-coreos@sha256:" + strings.Repeat("b", 64), PresentationSHA256: strings.Repeat("c", 64), HostPackagesSHA256: strings.Repeat("d", 64), Images: map[string]Image{}}
	p.ID = p.CoreOS + ".soda-" + p.Revision[:12]
	for i, n := range Names {
		d := Hash([]byte(n))
		p.Images[n] = Image{Reference: tr.Prefix + "-" + n + "@" + d, Manifest: d, Config: Hash([]byte{byte(i)}), ArchiveSHA256: strings.Repeat("1", 64)}
	}
	pb, e := marshal(p)
	require.NoError(t, e)
