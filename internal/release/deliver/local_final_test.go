...
func TestLocalSignedFinalMetadata(t *testing.T) {
	...
	require.NoError(t, build.WriteNew(evidence, []byte(`{"scope":"local-only","note":"retained-candidate B5 mechanism evidence; not protected P9"}`+"\n"), 0o600))

	n := Native{Home: out}
	require.NoError(t, CheckNative(t.Context(), n))
	trust, keys := syntheticReleaseTrust(t, n, out, stage)

	sum, err := build.HashFile(evidence)
	...
	digest, err := Prepare(trust, stage, mediaPath, q, prepared)
	require.NoError(t, err)
	signed := filepath.Join(out, "signed")
	permit := Permit{Format: 1, Repository: trust.Prefix + "-release", Digest: digest, Expires: time.Now().Add(time.Hour).Unix()}
	require.NoError(t, Sign(t.Context(), n, trust, permit, "oci", prepared, signed, keys))
	ref := permit.Repository + "@" + digest
	require.NoError(t, VerifyCopy(t.Context(), n, trust, ref, "dir:"+filepath.Join(signed, "signed"), filepath.Join(out, "verified")))

	var release Release
	require.NoError(t, ReadDocument(filepath.Join(out, "verified/image"), digest, &release))
	require.NotEmpty(t, release.Media)
	_, _, err = release.Validate(trust)
	require.NoError(t, err)

	require.Error(t, Sign(t.Context(), n, trust, Permit{Format: 1, Repository: trust.Prefix + "-release", Digest: Hash([]byte("wrong")), Expires: permit.Expires}, "oci", prepared, filepath.Join(out, "wrong-digest"), keys))
	...
	require.NoError(t, err)
	require.NoError(t, build.WriteNew(filepath.Join(out, "final.json"), append(data, '\n'), 0o600))
}

func stageCandidate(src, dest string) error {
	if err := build.FreshDirectory(dest); err != nil {
		return err
	}
	names := []string{
...
func writeReconstructedMedia(candidate, out string) error {
	...
	rootSum, err := build.HashFile(rootfsEntries[0])
	if err != nil {
		return err
	}
	...
	media := MediaBinding{
		...
		ISO:       MediaFile{Path: "minimal.iso", SHA256: isoSum, Bytes: isoStat.Size()},
		Rootfs:    MediaFile{Path: filepath.Base(rootfsEntries[0]), SHA256: rootSum, Bytes: rootStat.Size()},
	}
	data, err := json.MarshalIndent(media, "", "  ")
	if err != nil {
		return err
	}
	return build.WriteNew(out, append(data, '\n'), 0o600)
}

func syntheticReleaseTrust(t *testing.T, n Native, out, candidate string) (Trust, SecretFiles) {
	t.Helper()
	...
	for _, role := range []string{"artifact", "candidate", "preview", "stable"} {
		...
		require.NoError(t, err)
		pub, err := ReadFile(prefix+".pub", 16384)
		require.NoError(t, err)
		trust.Keys[role] = []string{string(pub)}
		if role == "artifact" {
			artifactKeys = SecretFiles{Key: prefix + ".private", Passphrase: pass}
		}
	}
...
func writePrivateJSON(path string, v any) error {
	...
	return build.WriteNew(path, append(data, '\n'), 0o600)
}
