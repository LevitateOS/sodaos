...
func (s *publicFileSet) visit(path string, d os.DirEntry, err error) error {
	...
	s.files[filepath.ToSlash(rel)], err = build.HashFile(path)
	return err
}
...
func validateCompletePayload(p deliver.Payload) error {
...
func writeCompleteQuadlets(source, root string, p deliver.Payload) error {
...
func Complete(source, context, archives string, p deliver.Payload, run build.BuildExec) error {
