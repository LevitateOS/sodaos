...
func linkCandidateCommands(source, context, native string) error {
	...
	commands, err := build.SodaCommands(source)
	if err != nil {
		return err
	}
...
func stageCandidateForgejo(p *deliver.Payload, source, context, out string, producer build.Production) (string, error) {
...
func recordCandidateImages(p *deliver.Payload, prefix string, images map[string]build.ProducedImage) {
	for name, im := range images {
		p.Images[name] = deliver.Image{Reference: prefix + "-" + name + "@" + im.Manifest, Manifest: im.Manifest, Config: im.Config, ArchiveSHA256: im.ArchiveSHA256}
	}
}

func inspectCandidateForgejo(source, out string, images map[string]build.ProducedImage, producer build.Production) error {
...
func sealCandidatePayload(p deliver.Payload, source, context, out string, producer build.Production) error {
	...
	return build.WriteNew(filepath.Join(out, "payload.json"), append(record, '\n'), 0o600)
}
