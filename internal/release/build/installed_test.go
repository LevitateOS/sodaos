...
func TestInstalledIdentityUsesExactBytesAndActivationPhase(t *testing.T) {
	for _, mode := range []string{"before", "activated", "coreos-absolute", "coreos-relative", "revision", "bytes", "mode", "hook-bytes", "hook-mode", "image", "inspection-error", "retired-tailnet", "retired-runners", "retired-native-tailnet"} {
		t.Run(mode, func(t *testing.T) {
			...
			inv := Inventory{Files: map[string]File{"rootfs/usr/local/libexec/soda/fixture": {SHA256: hash, Mode: 0o600}}, Images: map[string]Image{"forgejo": {Config: image}, "dashboard": {Config: image}, "caddy": {Config: image}}}
			hook := "var/lib/soda/forgejo/gitea/templates/custom/header.tmpl"
			if err := os.MkdirAll(filepath.Dir(filepath.Join(path, hook)), 0o755); err != nil {
				t.Fatal(err)
			}
			if err := os.WriteFile(filepath.Join(path, hook), []byte("fixture hook"), 0o644); err != nil {
				t.Fatal(err)
			}
			if err := os.Chmod(filepath.Join(path, hook), 0o644); err != nil {
				t.Fatal(err)
			}
			...
			inv.Files["rootfs/"+hook] = File{SHA256: hookHash, Mode: 0o644}
			if mode == "hook-bytes" {
				if err := os.WriteFile(filepath.Join(path, hook), []byte("changed hook"), 0o644); err != nil {
					t.Fatal(err)
				}
			}
			if mode == "hook-mode" {
				if err := os.Chmod(filepath.Join(path, hook), 0o600); err != nil {
					t.Fatal(err)
				}
			}
			if mode == "bytes" {
			...
			if mode == "activated" {
				if err := os.WriteFile(filepath.Join(path, "etc/soda/activated"), nil, 0o600); err != nil {
					t.Fatal(err)
				}
			}
			if strings.HasPrefix(mode, "coreos-") {
				if err := os.MkdirAll(filepath.Join(path, "var"), 0o755); err != nil {
					t.Fatal(err)
				}
			...
			if strings.HasPrefix(mode, "retired-") {
				...
				if e := os.MkdirAll(filepath.Join(path, retired), 0o755); e != nil {
					t.Fatal(e)
				}
			}
