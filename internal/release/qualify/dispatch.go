...
func hashDriver(executable string) (string, error) {
	...
	return build.HashFile(executable)
}
...
func admitDispatchArtifacts(c Config, candidate, revision string) (Artifact, Artifact, image.Media, string, error) {
	a, err := ReadArtifact(c.Baseline)
	if err != nil {
		return Artifact{}, Artifact{}, image.Media{}, "", err
	}
	b, err := ReadArtifact(candidate)
	if err != nil {
		return a, Artifact{}, image.Media{}, "", err
	}
	if b.Payload.Revision != revision {
		return a, b, image.Media{}, "", errors.New("candidate differs from frozen source revision")
	}
	if err = SameBaseScenario(a, b); err != nil {
		return a, b, image.Media{}, "", err
	}
...
func prepareDispatchCustody(c Config, custody string) (input string, uid, gid int, err error) {
	if err = build.FreshDirectory(custody); err != nil {
...
func hashCandidateMedia(candidate string, media image.Media, files map[string]string) error {
	for _, name := range []string{"media/media.json", "media/" + media.ISO.Path, "media/" + media.Rootfs.Path, "tools/soda-installer"} {
		h, e := build.HashFile(filepath.Join(candidate, name))
		if e != nil {
			return e
		}
		files[name] = h
...
func verifyCloneHash(work, want string) error {
	cloneSum, err := build.HashFile(filepath.Join(work, "a.qcow2"))
...
func snapshotDispatchInputs(ctx context.Context, c Config, candidate string, media image.Media, a, b Artifact, uid, gid int, input, sum string) (map[string]string, error) {
...
func verifyFrozenInputs(candidate, frozen string, files map[string]string, b Artifact, disk, sum string) error {
	for name, want := range files {
		got, e := build.HashFile(filepath.Join(frozen, name))
		if e != nil || got != want {
			return errors.New("qualification snapshot changed")
		}
		got, e = build.HashFile(filepath.Join(candidate, name))
		if e != nil || got != want {
...
func sealQualifiedReceipt(custody, driver string, a, b Artifact, media image.Media, result Receipt) error {
...
func finishDispatch(ctx context.Context, c Config, candidate, frozen, custody, driver, sum string, a, b Artifact, media image.Media, files map[string]string, log io.Writer) error {
	...
	var result Receipt
	if err := deliver.ReadJSON(filepath.Join(custody, "evidence/qualification.json"), &result); err != nil {
		return err
	}
