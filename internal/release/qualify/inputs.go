// Package qualify provides artifact admission and fixed guest-state
...
func loadArtifactMetadata(root string) (Artifact, error) {
	...
	a.Payload, err = deliver.Load(filepath.Join(root, "payload.json"))
	if err != nil {
		return a, err
	}
	if err = deliver.ReadJSON(filepath.Join(root, "candidate.json"), &a.Candidate); err != nil {
		return a, err
	}
...
func hashArtifactSidecars(opened *os.Root, files map[string]string) error {
	for _, path := range []string{"payload.json", "candidate.json"} {
		sum, e := build.HashAt(opened, path)
		if e != nil {
			return e
		}
		files[path] = sum
...
func ReadArtifact(root string) (Artifact, error) {
	...
	defer opened.Close()
	a.Files, err = deliver.VerifyCandidateImages(opened, root, a.Payload, a.Candidate)
	if err != nil {
		return a, err
	}
...
func mediaBindingMatches(m image.Media, a Artifact) bool {
...
func verifyMediaFile(root string, file image.MediaFile) error {
...
func ReadMedia(root string, a Artifact) (image.Media, error) {
	var m image.Media
	if err := deliver.ReadJSON(filepath.Join(root, "media/media.json"), &m); err != nil {
		return m, err
	}
	...
	for _, file := range []image.MediaFile{m.ISO, m.Rootfs} {
		if err := verifyMediaFile(root, file); err != nil {
			return m, err
		}
	}
	console, err := build.HashFile(filepath.Join(root, "tools/soda-installer"))
...
func Unchanged(root string, a Artifact) error {
	for path, want := range a.Files {
		got, err := build.HashFile(filepath.Join(root, path))
		if err != nil || got != want {
