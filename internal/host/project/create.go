...
func (r *Runtime) Create(ctx context.Context, in domain.Create) (domain.Environment, error) {
...
func (r *Runtime) createContainer(ctx context.Context, in domain.Create) error {
...
func applyCreationProfile(env *domain.Environment, item inspectItem) error {
	raw := item.Config.Labels["org.soda.creation-profile"]
	if raw == "" {
		return nil
	}
	p, decodeErr := domain.Decode(raw)
	image := item.Image
...
func admitProjectIP(env *domain.Environment, network, subnet string, networks map[string]struct{ IPAddress string }) error {
...
func (r *Runtime) Inspect(ctx context.Context, id string) (domain.Environment, int64, error) {
	env := domain.Environment{ID: id}
	...
	item := items[0]
	if domain.ValidImageRef(item.Image) {
...
func (r *Runtime) startCreated(ctx context.Context, in domain.Create) (domain.Environment, error) {
	name := "soda-" + in.ID
...
func confirmAccount(out []byte, in domain.Account) error {
	var result struct {
...
func (r *Runtime) Account(ctx context.Context, in domain.Account) error {
	if !domain.ValidLogin(in.Login) || in.Login == "root" || in.Identity <= 0 || len(in.Keys) > 32 {
...
func (r *Runtime) Connection(ctx context.Context, id string) (domain.Connection, error) {
	env, _, err := r.Inspect(ctx, id)
	result := domain.Connection{Environment: env}
	if err != nil {
		return result, err
	}
	if !env.Running {
