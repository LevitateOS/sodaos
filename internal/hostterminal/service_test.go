...
func terminalFixture(t *testing.T) (*Service, string, *terminalFake) {
	t.Helper()
...
func dialTerminal(t *testing.T, socket string) *http.Client {
	t.Helper()
...
func openTerminal(ctx context.Context, client *http.Client, in TerminalRequest) (*websocket.Conn, error) {
	if !in.Valid(time.Now()) {
		return nil, errors.New("invalid terminal request")
	}
	dialCtx, cancel := context.WithTimeout(ctx, 5*time.Second)
	defer cancel()
	conn, _, err := websocket.Dial(dialCtx, "ws://soda-host/terminal", &websocket.DialOptions{HTTPClient: client})
	if err != nil {
...
func receiveFrame(ctx context.Context, conn *websocket.Conn) (TerminalFrame, error) {
...
func sendFrame(ctx context.Context, conn *websocket.Conn, f TerminalFrame) error {
	if !f.InputValid() {
		return errors.New("invalid terminal control")
	}
	return Write(ctx, conn, f)
}
...
func TestTerminalStreamUsesVerifiedID(t *testing.T) {
	_, socket, f := terminalFixture(t)
	client := dialTerminal(t, socket)
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
	conn, err := openTerminal(ctx, client, terminalInput())
	if err != nil {
		t.Fatal(err)
	}
	defer conn.CloseNow()
	f.process.output <- TerminalFrame{Type: "ready"}
	got, err := receiveFrame(ctx, conn)
	...
	f.mu.Unlock()
	if err = sendFrame(ctx, conn, TerminalFrame{Type: "resize", Cols: 100, Rows: 30}); err != nil {
		t.Fatal(err)
	}
	select {
...
func TestTerminalBadFirstFrameNeverInspectsOrStarts(t *testing.T) {
	for _, body := range []string{`{"login":"root"}`, `{"project":"p0123456789abcdef01234567","command":"id"}`, `{"login":"alice","login":"bob"}`, `null`, strings.Repeat("x", 4097)} {
		t.Run(body[:min(len(body), 20)], func(t *testing.T) {
			...
			defer cancel()
			conn, _, err := websocket.Dial(ctx, "ws://soda-host/terminal", &websocket.DialOptions{HTTPClient: client})
			if err != nil {
				t.Fatal(err)
			}
			defer conn.CloseNow()
...
func TestTerminalShutdownClosesHijackedStreams(t *testing.T) {
	...
	defer conn.CloseNow()
	f.process.output <- TerminalFrame{Type: "ready"}
	if _, err = receiveFrame(ctx, conn); err != nil {
		t.Fatal(err)
	}
	...
	if _, err = openTerminal(ctx, client, terminalInput()); err == nil {
...
func TestTerminalCloseWaitsForAttachmentExit(t *testing.T) {
	...
	if err = sendFrame(ctx, conn, TerminalFrame{Type: "close"}); err != nil {
		t.Fatal(err)
	}
	select {
