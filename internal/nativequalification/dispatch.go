...
func Dispatch(ctx context.Context, c Config, candidate, revision, custody string, log io.Writer) error {
	...
	for source, name := range map[string]string{c.BaselineDisk: "a.qcow2", c.BaselineVariables: "a-vars.fd", c.BaselineKey: "a-key", c.BaselineKnownHosts: "a-known-hosts", c.BaselineState: "a-state.json"} {
	...
	if err = result.Validate(b, media.HostManifest); err != nil {
		return err
	}
