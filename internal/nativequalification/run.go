...
func loadRunInputs(configPath string) (Config, Artifact, Artifact, hostimage.Media, error) {
	var c Config
	if err := rd.ReadJSON(configPath, &c); err != nil {
		return c, Artifact{}, Artifact{}, hostimage.Media{}, err
	}
	if c.Work != "/run/soda-p9-work" {
		return c, Artifact{}, Artifact{}, hostimage.Media{}, errors.New("controller work mount required")
	}
	var a Artifact
	if err := rd.ReadJSON("/run/soda-p9-input/baseline.json", &a); err != nil {
		return c, a, Artifact{}, hostimage.Media{}, err
	}
	b, err := ReadArtifact("/run/soda-p9-input/candidate")
	if err != nil {
		return c, a, b, hostimage.Media{}, err
	}
	if err = SameBaseScenario(a, b); err != nil {
		return c, a, b, hostimage.Media{}, err
	}
	if b.Payload.RepositoryPrefix != "ghcr.io/levitateos/sodaos" {
		return c, a, b, hostimage.Media{}, errors.New("selected local qualification repository required")
	}
...
func sameAdmittedState(work string, before map[string]any) error {
	...
	if err := rd.ReadJSON(filepath.Join(work, "a-state.json"), &admitted); err != nil {
		return err
	}
...
func recoverQualifiedState(ctx context.Context, m *machine, f *fixture, c Config, a, b Artifact, media hostimage.Media, before map[string]any, password []byte, receipt *Receipt) error {
...
func runNativeScenario(ctx context.Context, c Config, a, b Artifact, media hostimage.Media, e *acceptance.Evidence, f *fixture, receipt *Receipt) (err error) {
