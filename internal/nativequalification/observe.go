...
func (r Receipt) Validate(a, b Artifact, host string) error {
	if r.Scope != "native-install-upgrade-recovery" || r.HostManifest != host || r.HostManifest != b.Candidate.Host.Manifest || r.BaselineHostManifest != a.Candidate.Host.Manifest || r.PayloadSHA256 != b.Candidate.PayloadSHA256 || r.BaselineCommit != r.RecoveredCommit || !nativebuild.Digest(r.BaselineCommit) || !nativebuild.Digest(r.InstallCommit) || !nativebuild.Digest(r.UpdatedCommit) {
