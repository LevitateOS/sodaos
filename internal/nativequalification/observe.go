...
func receiptDigests(r Receipt) bool {
	return nativebuild.Digest(r.BaselineCommit) && nativebuild.Digest(r.InstallCommit) && nativebuild.Digest(r.UpdatedCommit)
}
...
func loadGuestPayload(raw []byte, want appliancerelease.Payload) (appliancerelease.Payload, error) {
...
func healthScript(p appliancerelease.Payload, configured bool) string {
...
func (m *machine) verifyNativeContent(ctx context.Context, a Artifact, p appliancerelease.Payload, configured bool) error {
...
func (m *machine) driver(ctx context.Context, executable string) error {
	...
	hash, err := nativebuild.HashFile(executable)
