...
func validGuestAction(action string) bool {
	return action == "seed" || action == "later" || action == "snapshot"
}
...
func admitGuestState(action, expectedPayload string) (appliancerelease.Payload, error) {
	if !validGuestAction(action) {
		return appliancerelease.Payload{}, errors.New("fixed state action required")
	}
	if os.Geteuid() != 0 {
		return appliancerelease.Payload{}, errors.New("guest root required")
	}
	if _, err := os.Stat("/run/ostree-booted"); err != nil {
		return appliancerelease.Payload{}, errors.New("native OSTree guest required")
	}
...
func createFixtureAccount(ctx context.Context, credential string) error {
	...
	return nativebuild.WriteNew(credential, match[1], 0o600)
}
...
func updateFixtureGeneration(ctx context.Context, client forgejoClient, db *store.Store, userID int64, generation string, content []byte) error {
	...
	return db.UpsertUser(ctx, store.User{ID: userID, Login: fixtureLogin, Name: "Soda fixture generation " + generation})
}

func seedFixtureProject(ctx context.Context, db *store.Store, payload appliancerelease.Payload, user fixtureUser, repo fixtureRepo) error {
...
func mutateFixtureState(ctx context.Context, client forgejoClient, cfg guestConfig, payload appliancerelease.Payload, action string, user fixtureUser, repo fixtureRepo) error {
	...
	if err = updateFixtureGeneration(ctx, client, db, user.ID, generation, content); err != nil {
		return err
	}
	if action == "seed" {
...
func GuestState(ctx context.Context, action, expectedPayload string) (map[string]any, error) {
	...
	obs, err := observeGuestState(ctx, client, cfg.Database, user.ID)
	if err != nil {
		return nil, err
	}
