(() => {
  ...
  const content = panel.querySelector('.soda-notification-preview-content');
  const status = panel.querySelector('.soda-notification-preview-status');
  const retry = panel.querySelector('.soda-notification-preview-retry');
  const closeButton = panel.querySelector('.soda-notification-preview-close');
  const destination = panel.querySelector('.soda-notification-preview-all').href;
  const bells = [...document.querySelectorAll('#navbar a')].filter(link =>
  ...
  document.addEventListener('focusin', event => {
    if (panel.matches(':popover-open') && !panel.contains(event.target) && !bells.includes(event.target)) close(false);
  });
