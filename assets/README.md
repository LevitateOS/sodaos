...
Raster exports and manifests are retained unchanged. Regeneration tooling remains in the source repository (`scripts/render-branding.*`, `scripts/render-installer-branding.sh`, `scripts/render-forgejo-branding.sh`, and `tools/render-cockpit-branding/`). Regenerate there until that tooling is deliberately ported; do not hand-edit derivative PNGs.
